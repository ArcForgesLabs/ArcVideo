...
ARCVIDEO_ADD_TEST(AddTrack) {
...

    {
        ...
        ARCVIDEO_ASSERT(sequence.GetConnectedOutput(Sequence::kTextureInput) == first_video_track);
        ARCVIDEO_ASSERT(sequence.track_list(Track::kVideo)->GetTrackCount() == 1);
        ARCVIDEO_ASSERT(sequence.track_list(Track::kVideo)->GetTrackAt(0) == first_video_track);
    }

    {
        ...
        ARCVIDEO_ASSERT(sequence.GetConnectedOutput(Sequence::kSamplesInput) == first_audio_track);
        ARCVIDEO_ASSERT(sequence.track_list(Track::kAudio)->GetTrackCount() == 1);
        ARCVIDEO_ASSERT(sequence.track_list(Track::kAudio)->GetTrackAt(0) == first_audio_track);
    }

    {
        ...
        ARCVIDEO_ASSERT(sequence.GetConnectedOutput(Sequence::kTextureInput) != first_video_track);
        ARCVIDEO_ASSERT(sequence.GetConnectedOutput(Sequence::kTextureInput) != second_video_track);
        ARCVIDEO_ASSERT(sequence.track_list(Track::kVideo)->GetTrackCount() == 2);
        ARCVIDEO_ASSERT(sequence.track_list(Track::kVideo)->GetTrackAt(1) == second_video_track);

        MergeNode* merge = dynamic_cast<MergeNode*>(sequence.GetConnectedOutput(Sequence::kTextureInput));
        ARCVIDEO_ASSERT(merge);
        ARCVIDEO_ASSERT(merge->GetConnectedOutput(MergeNode::kBaseIn) == first_video_track);
        ARCVIDEO_ASSERT(merge->GetConnectedOutput(MergeNode::kBlendIn) == second_video_track);
    }

    {
        ...
        ARCVIDEO_ASSERT(sequence.GetConnectedOutput(Sequence::kSamplesInput) != first_audio_track);
        ARCVIDEO_ASSERT(sequence.GetConnectedOutput(Sequence::kSamplesInput) != second_audio_track);
        ARCVIDEO_ASSERT(sequence.track_list(Track::kAudio)->GetTrackCount() == 2);
        ARCVIDEO_ASSERT(sequence.track_list(Track::kAudio)->GetTrackAt(1) == second_audio_track);

        MathNode* merge = dynamic_cast<MathNode*>(sequence.GetConnectedOutput(Sequence::kSamplesInput));
        ARCVIDEO_ASSERT(merge);
        ARCVIDEO_ASSERT(merge->GetConnectedOutput(MathNode::kParamAIn) == first_audio_track);
        ARCVIDEO_ASSERT(merge->GetConnectedOutput(MathNode::kParamBIn) == second_audio_track);
    }

    ARCVIDEO_TEST_END;
}

ARCVIDEO_ADD_TEST(SequenceDefaults) {
    TIMELINE_TEST_START;

    sequence.add_default_nodes();

    ARCVIDEO_ASSERT(sequence.GetTracks().size() == 2);

    Track* tex_connect = dynamic_cast<Track*>(sequence.GetConnectedTextureOutput());
    ARCVIDEO_ASSERT(tex_connect);
    Track* smp_connect = dynamic_cast<Track*>(sequence.GetConnectedSampleOutput());
    ARCVIDEO_ASSERT(smp_connect);
    ARCVIDEO_ASSERT(tex_connect != smp_connect);
    ARCVIDEO_ASSERT(sequence.GetTracks().contains(tex_connect));
    ARCVIDEO_ASSERT(sequence.GetTracks().contains(smp_connect));

    ARCVIDEO_TEST_END;
}

ARCVIDEO_ADD_TEST(Trim) {
    TIMELINE_TEST_START;
    ...
    ARCVIDEO_ASSERT(track->Blocks().size() == 2);

    {
        ...
        ARCVIDEO_ASSERT(track->Blocks().size() == 2);
        ARCVIDEO_ASSERT(block2->length() == 1);
        ARCVIDEO_ASSERT(block1->length() == 2);

        command.undo_now();

        ARCVIDEO_ASSERT(track->Blocks().size() == 2);
        ARCVIDEO_ASSERT(block2->length() == 2);
        ARCVIDEO_ASSERT(block1->length() == 2);
    }

    {
        ...
        ARCVIDEO_ASSERT(track->Blocks().size() == 3);
        GapBlock* gap = dynamic_cast<GapBlock*>(track->Blocks().at(1));
        ARCVIDEO_ASSERT(gap);
        ARCVIDEO_ASSERT(gap->length() == 1);
        ARCVIDEO_ASSERT(block2->length() == 1);
        ARCVIDEO_ASSERT(block1->length() == 2);
        ARCVIDEO_ASSERT(block1->next() == gap);
        ARCVIDEO_ASSERT(block2->previous() == gap);
...

    {
        ...
        ARCVIDEO_ASSERT(gap);
        ARCVIDEO_ASSERT(gap->length() == 1);
        ARCVIDEO_ASSERT(block1->length() == 1);
        ARCVIDEO_ASSERT(block2->length() == 2);
        ARCVIDEO_ASSERT(block1->next() == gap);
        ARCVIDEO_ASSERT(block2->previous() == gap);
...

    {
        ...
        ARCVIDEO_ASSERT(block1->length() == 1);
        ARCVIDEO_ASSERT(block2->length() == 2);
        ARCVIDEO_ASSERT(block1->next() == block2);
        ARCVIDEO_ASSERT(block1->previous() == gap);
...
ARCVIDEO_ADD_TEST(ReplaceBlockWithGap_ClipsOnly) {
    TIMELINE_TEST_START;
...

    {
        ...
        // end of the track
        ARCVIDEO_ASSERT(track->Blocks().size() == 2);
        ARCVIDEO_ASSERT(track->Blocks().at(0) == a);
        ARCVIDEO_ASSERT(track->Blocks().at(1) == b);
        ...
        ARCVIDEO_ASSERT(track->Blocks().at(0) == a);
        ARCVIDEO_ASSERT(track->Blocks().at(1) == b);
        ARCVIDEO_ASSERT(track->Blocks().at(2) == c);
    }

    {
        ...
        ARCVIDEO_ASSERT(track->Blocks().at(0) == a);
        ARCVIDEO_ASSERT(track->Blocks().at(1) != b);
        ARCVIDEO_ASSERT(dynamic_cast<GapBlock*>(track->Blocks().at(1)));
        ARCVIDEO_ASSERT(track->Blocks().at(1)->length() == b->length());
        ARCVIDEO_ASSERT(track->Blocks().at(2) == c);
...
ARCVIDEO_ADD_TEST(ReplaceBlockWithGap_ClipsAndGaps) {
    TIMELINE_TEST_START;
...

    {
        ...
        ARCVIDEO_ASSERT(track->Blocks().size() == 5);
        ARCVIDEO_ASSERT(track->Blocks().at(0) == a);
        ARCVIDEO_ASSERT(track->Blocks().at(1) == b);
        ARCVIDEO_ASSERT(track->Blocks().at(2) == c);
        ARCVIDEO_ASSERT(track->Blocks().at(3) == d);
        ARCVIDEO_ASSERT(track->Blocks().at(4) == e);
    }

    {
        ...
        ARCVIDEO_ASSERT(track->Blocks().size() == 4);

        ARCVIDEO_ASSERT(track->Blocks().at(0) == b);
        ARCVIDEO_ASSERT(track->Blocks().at(1) == c);
        ARCVIDEO_ASSERT(track->Blocks().at(2) == d);
        ARCVIDEO_ASSERT(track->Blocks().at(3) == e);
        ARCVIDEO_ASSERT(b->length() == original_length_of_a + original_length_of_b);
        ...
        ARCVIDEO_ASSERT(track->Blocks().at(3) == d);
        ARCVIDEO_ASSERT(track->Blocks().at(4) == e);
        ARCVIDEO_ASSERT(a->length() == original_length_of_a);
        ARCVIDEO_ASSERT(b->length() == original_length_of_b);
    }

    {
        ...
        ARCVIDEO_ASSERT(track->Blocks().at(0) == a);
        ARCVIDEO_ASSERT(track->Blocks().at(1) == b);
        ARCVIDEO_ASSERT(track->Blocks().at(2) == e);
        ARCVIDEO_ASSERT(b->length() == original_length_of_b + original_length_of_c + original_length_of_d);
        ...
        ARCVIDEO_ASSERT(track->Blocks().at(4) == e);
        ARCVIDEO_ASSERT(b->length() == original_length_of_b);
        ARCVIDEO_ASSERT(c->length() == original_length_of_c);
        ARCVIDEO_ASSERT(d->length() == original_length_of_d);
    }

    {
        ...
        ARCVIDEO_ASSERT(track->Blocks().at(2) == c);
        ARCVIDEO_ASSERT(track->Blocks().at(3) == d);
        ARCVIDEO_ASSERT(track->Blocks().at(4) == f);
        ARCVIDEO_ASSERT(d->length() == original_length_of_d + original_length_of_e);

        command.undo_now();

        ARCVIDEO_ASSERT(track->Blocks().size() == 6);
        ARCVIDEO_ASSERT(track->Blocks().at(0) == a);
        ARCVIDEO_ASSERT(track->Blocks().at(1) == b);
        ARCVIDEO_ASSERT(track->Blocks().at(2) == c);
        ARCVIDEO_ASSERT(track->Blocks().at(3) == d);
        ARCVIDEO_ASSERT(track->Blocks().at(4) == e);
        ARCVIDEO_ASSERT(track->Blocks().at(5) == f);
        ARCVIDEO_ASSERT(d->length() == original_length_of_d);
        ARCVIDEO_ASSERT(e->length() == original_length_of_e);
    }
...
ARCVIDEO_ADD_TEST(ReplaceBlockWithGap_ClipsAndTransitions) {
    TIMELINE_TEST_START;
...

    {
        ...
        ARCVIDEO_ASSERT(track->Blocks().size() == 4);
        ARCVIDEO_ASSERT(dynamic_cast<GapBlock*>(track->Blocks().at(0)));
        ARCVIDEO_ASSERT(track->Blocks().at(1) == a_to_b);
        ARCVIDEO_ASSERT(track->Blocks().at(2) == b);
        ARCVIDEO_ASSERT(track->Blocks().at(3) == b_out);

        command.undo_now();

        ARCVIDEO_ASSERT(track->Blocks().size() == 5);
        ARCVIDEO_ASSERT(track->Blocks().at(0) == a_in);
        ARCVIDEO_ASSERT(track->Blocks().at(1) == a);
        ARCVIDEO_ASSERT(track->Blocks().at(2) == a_to_b);
        ARCVIDEO_ASSERT(track->Blocks().at(3) == b);
        ARCVIDEO_ASSERT(track->Blocks().at(4) == b_out);
    }

    ARCVIDEO_TEST_END;
}

ARCVIDEO_ADD_TEST(InsertGaps_SingleTrack) {
    TIMELINE_TEST_START;
...

    {
        ...
        ARCVIDEO_ASSERT(track->Blocks().size() == 4);
        ARCVIDEO_ASSERT(dynamic_cast<GapBlock*>(track->Blocks().at(0)));
        ARCVIDEO_ASSERT(track->Blocks().at(0)->length() == 2);
        ARCVIDEO_ASSERT(track->Blocks().at(1) == a);
        ARCVIDEO_ASSERT(track->Blocks().at(2) == b);
        ARCVIDEO_ASSERT(track->Blocks().at(3) == c);
...

    {
        ...
        ARCVIDEO_ASSERT(track->Blocks().size() == 5);
        ARCVIDEO_ASSERT(track->Blocks().at(0) == a);
        ARCVIDEO_ASSERT(track->Blocks().at(0)->length() == rational(1, 2));
        ARCVIDEO_ASSERT(dynamic_cast<GapBlock*>(track->Blocks().at(1)));
        ARCVIDEO_ASSERT(dynamic_cast<ClipBlock*>(track->Blocks().at(2)));
        ARCVIDEO_ASSERT(track->Blocks().at(3) == b);
        ARCVIDEO_ASSERT(track->Blocks().at(4) == c);

        command.undo_now();

        ARCVIDEO_ASSERT_EQUAL(track->Blocks().size(), 3);
        ARCVIDEO_ASSERT_EQUAL(track->Blocks().at(0), a);
        ARCVIDEO_ASSERT_EQUAL(track->Blocks().at(0)->length(), 1);
        ARCVIDEO_ASSERT_EQUAL(track->Blocks().at(1), b);
        ARCVIDEO_ASSERT_EQUAL(track->Blocks().at(2), c);
    }
