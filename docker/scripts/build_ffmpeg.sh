...
# Set up libsvtav1
{
    ...
    cmake .. \
      -G "Unix Makefiles" \
      -DCMAKE_INSTALL_PREFIX="${ARCVIDEO_INSTALL_PREFIX}" \
...
# Set up libx264
{
    ...
    cd x264
    ./configure \
      --prefix="${ARCVIDEO_INSTALL_PREFIX}" \
      --enable-shared \
      --enable-pic \
      --disable-cli
    make
...
# Set up libvorbis
{
    ...
    ./configure \
      --prefix="${ARCVIDEO_INSTALL_PREFIX}" \
      --with-ogg="${ARCVIDEO_INSTALL_PREFIX}" \
      --enable-shared
    make
    make install
    cd ..
...
### Set up xvid
{
    ...
    ./configure \
      --prefix="${ARCVIDEO_INSTALL_PREFIX}" \
      --bindir="${ARCVIDEO_INSTALL_PREFIX}/bin"
    make
    make install
    cd ../../..
    rm -rf xvidcore
...
PKG_CONFIG_PATH="/usr/local/lib/pkgconfig:/usr/local/lib64/pkgconfig:$PKG_CONFIG_PATH" ./configure \
  ...
  --extra-libs=-lpthread \
  --extra-libs=-lm \
  --extra-cflags="-I${ARCVIDEO_INSTALL_PREFIX}/include" \
  --extra-ldflags="-L${ARCVIDEO_INSTALL_PREFIX}/lib"
make -j$(nproc)
make install
cd ..
rm -rf ffmpeg*
