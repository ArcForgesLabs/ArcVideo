...
mkdir crashpad
cd crashpad
git clone --depth 1 https://github.com/arcvideo-editor/crashpad.git
gclient config https://github.com/arcvideo-editor/crashpad.git
gclient sync
cd crashpad
...
rsync -av \
    ...
    --prune-empty-dirs \
    . "${ARCVIDEO_INSTALL_PREFIX}/crashpad"
...
fetch breakpad
cd src
./configure --prefix="${ARCVIDEO_INSTALL_PREFIX}"
make -j$(nproc)
make install
