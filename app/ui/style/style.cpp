...
const char* StyleManager::kDefaultStyle = "arcvideo-dark";
...
void StyleManager::Init() {
    ...
    available_themes_.insert(QStringLiteral("arcvideo-dark"), QStringLiteral("ArcVideo Dark"));
    available_themes_.insert(QStringLiteral("arcvideo-light"), QStringLiteral("ArcVideo Light"));

    QString config_style = ARCVIDEO_CONFIG("Style").toString();
...
void StyleManager::SetStyle(const QString& style_path) {
    ...
    // Load all icons for this style (icons must be loaded first because the style change below triggers the icon
    // change)
