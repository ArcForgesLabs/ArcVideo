...
void Config::SetDefaults() {
    ...
    SetEntryInternal(QStringLiteral("DefaultVideoTransition"), NodeValue::kText,
                     QStringLiteral("org.arcvideoeditor.ArcVideo.crossdissolve"));
        ...
        if (reader.name() == QStringLiteral("Configuration")) {
            while (XMLReadNextStartElement(&reader)) {
                ...
                if (key == QStringLiteral("Version")) {
                    ...
                    // 0.1.x stored this value as a float while we now use rationals, we'll use a heuristic to find the
                    // closest supported rational
                    ...
                    current_config_[key] =
                        NodeValue::StringToValue(current_config_.GetConfigEntryType(key), value, false);
                }
            }
