...
QVariant OpenGLRenderer::CreateNativeTexture(int width, int height, int depth, PixelFormat format, int channel_count,
    ...
    if (is_3d) {
        context_->extraFunctions()->glTexImage3D(target, 0, GetInternalFormat(format, channel_count), width, height,
                                                 depth, 0, GetPixelFormat(channel_count), GetPixelType(format), data);
    } else {
...
void OpenGLRenderer::Flush() {
    ...
    if (ARCVIDEO_CONFIG("UseGLFinish").toBool()) {
        functions_->glFinish();
    } else {
        functions_->glFlush();
    }
        ...
        switch (value.type()) {
            ...
            case NodeValue::kTexture: {
                ...
                int enable_param_location = functions_->glGetUniformLocation(
                    shader, QStringLiteral("%1_enabled").arg(it.key()).toUtf8().constData());
