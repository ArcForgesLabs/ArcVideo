...
/** \mainpage ArcVideo Video Editor - Code Documentation
 *
 * This documentation is a primarily a developer resource. For information on using ArcVideo, visit the website
 * https://www.arcvideoeditor.org/
 *
...
int decompress_project(const QString &project)
{
  ...
  if (!arcvideo::ProjectSerializer::CheckCompressedID(&project_file)) {
...
int main(int argc, char *argv[])
{
  ...
  qInstallMessageHandler(arcvideo::DebugHandler);

  // Set application metadata
  QCoreApplication::setOrganizationName("arcvideoeditor.org");
  QCoreApplication::setOrganizationDomain("arcvideoeditor.org");
  QCoreApplication::setApplicationName("ArcVideo");
  QGuiApplication::setDesktopFileName("org.arcvideoeditor.ArcVideo");
  QCoreApplication::setApplicationVersion(arcvideo::kAppVersionLong);
  ...
  arcvideo::Core::CoreParams startup_params;
  ...
  if (export_option->IsSet()) {
    startup_params.set_run_mode(arcvideo::Core::CoreParams::kHeadlessExport);
  }
  ...
  if (startup_params.run_mode() == arcvideo::Core::CoreParams::kRunNormal) {
#ifdef _WIN32
    // Since ArcVideo is linked with the console subsystem (for better POSIX compatibility), a console
  ...
  if (!has_proc_address) {
    QString msg = QCoreApplication::translate("main",
      ...
      "This most likely means either your graphics driver is not up-to-date or your graphics card is too old to run ArcVideo.\n\n"
  ...
  // Start core
  arcvideo::Core c(startup_params);
