...
MainWindow::MainWindow(QWidget* parent)
    ...
    //       emit the "isOpenChanged" signal before emitting other signals, resulting
    //       in Core thinking there are -1 pixel samplers open. To mitigate that,
    //       we force "isOpenChanged" to emit ourselves here.
...
void MainWindow::ToggleMaximizedPanel() {
    ...
    if (premaximized_state_.isEmpty()) {
        ...
        currently_focused_panel->QWidget::setFocus();
...
void MainWindow::SetProject(Project* p) {
    ...
    if (project_) {
        project_panel_->QWidget::setFocus();
    }
}
...
void MainWindow::ShowWelcomeDialog() {
    if (ARCVIDEO_CONFIG("ShowWelcomeDialog").toBool()) {
...
void MainWindow::ShowNouveauWarning() {
    QMessageBox::warning(this, tr("Driver Warning"),
                         tr("ArcVideo has detected your system is using the Nouveau graphics driver.\n\nThis driver is "
                            "known to have stability and performance issues with ArcVideo. It is highly recommended "
                            "you install the proprietary NVIDIA driver before continuing to use ArcVideo."),
                         QMessageBox::Ok);
}
#endif
...
void MainWindow::UpdateTitle() {
    if (Core::instance()->GetActiveProject()) {
        ...
        setWindowTitle(
            QStringLiteral("%1 %2").arg(QApplication::applicationName(), QApplication::applicationVersion()));
    }
}

void MainWindow::TimelineCloseRequested() {
    auto* t = static_cast<TimelinePanel*>(sender());
    RemoveTimelinePanel(t);
}

void MainWindow::ViewerCloseRequested() {
    auto* panel = static_cast<ViewerPanel*>(sender());
...
void MainWindow::ViewerWithPanelRemovedFromGraph() {
    auto* vo = static_cast<ViewerOutput*>(sender());
...
void MainWindow::FolderPanelCloseRequested() {
    auto* panel = static_cast<ProjectPanel*>(sender());
...
#ifdef Q_OS_LINUX
        // Check for nouveau since that driver really doesn't work with ArcVideo
