...
void NodeView::DeleteSelected() {
    auto* command = new NodeViewDeleteCommand();
...
void NodeView::Select(const QVector<Node::ContextPair>& nodes, bool center_view_on_item) {
    ...
    if (center_view_on_item && !nodes.isEmpty()) {
        QMetaObject::invokeMethod(this, "CenterOnNode", Qt::QueuedConnection,
                                  ARCVIDEO_NS_ARG(Node*, nodes.first().node));
    }
...
void NodeView::Duplicate() {
    if (!selected_nodes_.isEmpty()) {
        ...
        for (int i = 0; i < selected.size(); i++) {
            ...
            if (auto* g = dynamic_cast<NodeGroup*>(selected.at(i))) {
        ...
        for (int i = 0; i < new_nodes.size(); i++) {
            ...
            if (auto* src_group = dynamic_cast<NodeGroup*>(og)) {
                auto* dst_group = static_cast<NodeGroup*>(copy);

                for (auto it = src_group->GetInputPassthroughs().cbegin();
                     it != src_group->GetInputPassthroughs().cend(); it++) {
...
void NodeView::keyPressEvent(QKeyEvent* event) {
    switch (event->key()) {
        ...
        case Qt::Key_Up:
        case Qt::Key_Down: {
            auto* pos_command = new MultiUndoCommand();
...
void NodeView::mousePressEvent(QMouseEvent* event) {
    ...
    if (event->button() == Qt::LeftButton) {
        ...
        if (event->modifiers() & Qt::ControlModifier) {
            auto* mouse_item = dynamic_cast<NodeViewItem*>(item);
        ...
        if (!create_edge_output_item_ && !create_edge_input_item_) {
            ...
            if (auto* connector = dynamic_cast<NodeViewItemConnector*>(item)) {
                auto* attached = static_cast<NodeViewItem*>(connector->parentItem());
...
void NodeView::mouseDoubleClickEvent(QMouseEvent* event) {
    ...
    if (!(event->modifiers() & Qt::ControlModifier)) {
        auto* item_at_cursor = dynamic_cast<NodeViewItem*>(itemAt(event->position().toPoint()));
        if (item_at_cursor) {
...
void NodeView::dragEnterEvent(QDragEnterEvent* event) {
    ...
    if (mime_fmts.contains(Project::kItemMimeType)) {
        ...
        while (!stream.atEnd()) {
            ...
            if (auto* f = dynamic_cast<ViewerOutput*>(item)) {
...
void NodeView::ShowContextMenu(const QPoint& pos) {
    ...
    if (itemAt(pos) && !selected.isEmpty()) {
        ...
        if (auto* viewer = dynamic_cast<ViewerOutput*>(selected.first()->GetNode())) {
            Q_UNUSED(viewer)
            m.addSeparator();
...
void NodeView::CreateNodeSlot(QAction* action) {
    ...
    if (new_node) {
        auto* new_item = new NodeViewItem(new_node, nullptr);
        new_item->SetFlowDirection(scene_.GetFlowDirection());
        scene_.addItem(new_item);
        ...
        if (auto* new_group = dynamic_cast<NodeGroup*>(new_node)) {
            for (auto it = new_group->GetContextPositions().cbegin(); it != new_group->GetContextPositions().cend();
                 it++) {
...
void NodeView::OpenSelectedNodeInViewer() {
    ...
    for (Node* n : selected_nodes_) {
        if (auto* viewer = dynamic_cast<ViewerOutput*>(n)) {
...
QVector<Node*> NodeView::ProcessDroppingAttachedNodes(MultiUndoCommand* command, Node* select_context,
    ...
    {
        auto* add_command = new MultiUndoCommand();
...

    {
        ...
        auto* drop_edge_command = new MultiUndoCommand();
        if (attached.size() == 1) {
...
Node* NodeView::GetContextAtMousePos(const QPoint& p) {
    ...
    for (QGraphicsItem* i : items_at_cursor) {
        if (auto* context_item = dynamic_cast<NodeViewContext*>(i)) {
...
void NodeView::ZoomIntoCursorPosition(QWheelEvent* event, double multiplier, const QPointF& cursor_pos) {
    ...
    if (test_scale > kMinimumScale) {
        ...
        int anchor_y =
            qRound(double(cursor_pos.y() + verticalScrollBar()->value()) / scale_ * test_scale - cursor_pos.y());
...
bool NodeView::event(QEvent* event) {
    if (event->type() == QEvent::ShortcutOverride) {
        auto* se = static_cast<QKeyEvent*>(event);
...
void NodeView::PositionNewEdge(const QPoint& pos) {
    ...
    auto* item_at_cursor = dynamic_cast<NodeViewItem*>(itemAt(pos));
...
void NodeView::UngroupNodes() {
    ...
    for (auto it = group->GetContextPositions().cbegin(); it != group->GetContextPositions().cend(); it++) {
        ...
        command->add_child(
            new NodeSetPositionCommand(it.key(), context, group->GetNodePositionDataInContext(it.key())));
    }
...
void NodeView::ShowNodeProperties() {
    ...
    if (auto* group = dynamic_cast<NodeGroup*>(first_node)) {
        if (!overlay_view_) {
            overlay_view_ = new NodeView(this);
            overlay_view_->show();

            auto* overlay_close_btn = new QPushButton(overlay_view_);
...
void NodeView::EndEdgeDrag(bool cancel) {
    ...
    if (create_edge_output_item_ && create_edge_input_item_ && !cancel) {
        if (creating_input.IsValid()) {
            ...
            if (!reconnected_to_itself) {
                ...
                while (auto* output_group = dynamic_cast<NodeGroup*>(creating_output)) {
                    creating_output = output_group->GetOutputPassthrough();
                }

                while (auto* input_group = dynamic_cast<NodeGroup*>(creating_input.node())) {
                    creating_input = input_group->GetInputFromID(creating_input.input());
                }

                if (creating_input.IsConnected()) {
                    Node::OutputConnection existing_edge_to_remove = {creating_input.GetConnectedOutput(),
                                                                      creating_input};
                    ...
                    if (ctx && !ctx->GetItemFromMap(already_connected_output)) {
                        if (QMessageBox::warning(
                                this, QString(),
                                tr("Input \"%1\" is currently connected to node \"%2\", which is not visible in this "
                                   "context. "
                ...
                if (!cancel) {
                    ...
                    if (!scene_.context_map()
                             ...
                             ->GetItemFromMap(creating_output)) {
    ...
    for (auto create_edge_expanded_item : std::views::reverse(create_edge_expanded_items_)) {
...
NodeViewContext* NodeView::GetContextItemFromNodeItem(NodeViewItem* item) {
    ...
    while ((i = i->parentItem())) {
        if (auto* nvc = dynamic_cast<NodeViewContext*>(i)) {
            return nvc;
        }
    }
    return nullptr;
