...
HistogramScope::HistogramScope(QWidget* parent) : super(parent) {}
...
ShaderCode HistogramScope::GenerateShaderCode() {
    return {FileFunctions::ReadFileAsString(":/shaders/rgbhistogram.frag"),
...
void HistogramScope::DrawScope(TexturePtr managed_tex, QVariant pipeline) {
    ...
    if (!texture_row_sums_ || texture_row_sums_->width() != this->width() ||
        ...
        texture_row_sums_ = renderer()->CreateTexture(
            VideoParams(width(), height(), managed_tex->format(), managed_tex->channel_count()));
    }
    ...
    for (auto it = histogram_increments.begin(); it != histogram_increments.end(); it++) {
        ...
        p.drawText(histogram_start_dim_x - font_x_offset,
                   (histogram_dim_y * pow(1.0 - *it, histogram_base)) + histogram_start_dim_y + font_y_offset, label);
    }
