...
void MulticamDisplay::OnPaint() {
    ...
    if (node_) {
        ...
        int col, row;
        arcvideo::MultiCamNode::IndexToRowCols(node_->GetCurrentSource(), rows, cols, &row, &col);
...
QString MulticamDisplay::GenerateShaderCode(int rows, int cols) {
    ...
    for (int x = 0; x < cols; x++) {
        for (int y = 0; y < rows; y++) {
            ...
            shader.append(
                QStringLiteral("uniform bool tex_%1_%2_enabled;").arg(QString::number(y), QString::number(x)));
        }
    }
    ...
    for (int x = 0; x < cols; x++) {
        ...
        if (x == cols - 1) {
            ...
            shader.append(
                QStringLiteral("  if (ove_texcoord.x < %1) {").arg(dblToGlsl(double(x + 1) / double(multiplier))));
        }
        ...
        for (int y = 0; y < rows; y++) {
            ...
            if (y == rows - 1) {
                ...
                shader.append(QStringLiteral("    if (ove_texcoord.y < %1) {")
