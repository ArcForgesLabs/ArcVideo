...
void ProjectExplorer::AddView(QAbstractItemView* view) {
    ...
    connect(view->selectionModel(), &QItemSelectionModel::selectionChanged, this,
            &ProjectExplorer::ViewSelectionChanged);
...
void ProjectExplorer::RevealSelectedFootage() {
    auto* footage = static_cast<Footage*>(context_menu_items_.first());
...
void ProjectExplorer::ContextMenuStartProxy(QAction* a) {
    ...
    for (Node* item : context_menu_items_) {
        auto* f = static_cast<Footage*>(item);
...
void ProjectExplorer::ViewSelectionChanged() {
    auto* model = static_cast<QItemSelectionModel*>(sender());
