...
QModelIndex ProjectViewModel::index(int row, int column, const QModelIndex& parent) const {
    ...
    auto* item_parent = static_cast<Folder*>(GetItemObjectFromIndex(parent));
    ...
    switch (role) {
        ...
        case kInnerTextRole: {
            ...
            switch (column_type) {
                ...
                case kLastModified:
                case kCreatedTime: {
                    qint64 using_time = (column_type == kLastModified)
    ...
    if (index.isValid() && index.column() == kName && role == Qt::EditRole) {
        ...
        if (!new_name.isEmpty()) {
            ...
            Core::instance()->undo_stack()->push(nrc,
                                                 tr(R"(Renamed Item "%1" to "%2")").arg(item->GetLabel(), new_name));
...
QMimeData* ProjectViewModel::mimeData(const QModelIndexList& indexes) const {
    ...
    // The indexes list includes indexes for each column which we don't use. To make sure each row only gets sent
    // *once*, we keep a list of dragged items
...
bool ProjectViewModel::dropMimeData(const QMimeData* data, Qt::DropAction action, int row, int column,
    ...
    if (mime_formats.contains(Project::kItemMimeType)) {
        ...
        while (!stream.atEnd()) {
            ...
            if (item != drop_location && item->folder() != drop_location &&
                ...
                move_command->add_child(new NodeEdgeRemoveCommand(
                    item,
                    NodeInput(item->folder(), Folder::kChildInput, item->folder()->index_of_child_in_array(item))));
        ...
        // If we didn't drop onto an item, find the nearest parent folder (should eventually terminate at root either
        // way)
