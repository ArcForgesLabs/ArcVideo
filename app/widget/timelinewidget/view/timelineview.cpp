        ...
        if (b) {
            setToolTip(tr("In: %1\nOut: %2\nDuration: %3")
                           .arg(QString::fromStdString(Timecode::time_to_timecode(
                                    b->in(), timebase(), Core::instance()->GetTimecodeDisplay())),
                                QString::fromStdString(Timecode::time_to_timecode(
                                    b->out(), timebase(), Core::instance()->GetTimecodeDisplay())),
                                QString::fromStdString(Timecode::time_to_timecode(
                                    b->length(), timebase(), Core::instance()->GetTimecodeDisplay()))));
        } else {
            setToolTip(QString());
        }
    }
...
void TimelineView::dragEnterEvent(QDragEnterEvent* event) {
    TimelineViewMouseEvent timeline_event =
        CreateMouseEvent(event->position().toPoint(), Qt::NoButton, event->modifiers());
    ...
    if (ghosts_ && !ghosts_->isEmpty()) {
        for (TimelineViewGhostItem* ghost : (*ghosts_)) {
            if (ghost->GetTrack().type() == connected_track_list_->type() && !ghost->IsInvisible()) {
                ...
                if (attached && ARCVIDEO_CONFIG("ShowClipWhileDragging").toBool()) {
...
TimelineViewMouseEvent TimelineView::CreateMouseEvent(const QPoint& pos, Qt::MouseButton button,
    ...
    return TimelineViewMouseEvent(scene_pt, pos, GetScale(), timebase(),
                                  Track::Reference(ConnectedTrackType(), SceneToTrack(scene_pt.y())), button,
                                  modifiers);
}
        ...
        if (r.width() <= MINIMUM_RECT_WIDTH) {
            ...
            if (foreground) {
                ...
                if (r.width() > MINIMUM_DETAIL_WIDTH) {
                    ...
                    painter->setPen(block->is_enabled() ? ColorCoding::GetUISelectorColor(block->color())
                                                        : Qt::lightGray);
                ...
                if (r.width() > MINIMUM_DETAIL_WIDTH) {
                    if (auto* clip = dynamic_cast<ClipBlock*>(block)) {
                        ...
                        if (clip->GetTrackType() == Track::kVideo &&
                            ARCVIDEO_CONFIG("TimelineThumbnailMode").toInt() != Timeline::kThumbnailOff) {
                            ...
                            if (preview_rect.height() > r.height() / 3) {
                                if (const FrameHashCache* thumbs = clip->thumbnails()) {
                                    ...
                                    if (ARCVIDEO_CONFIG("TimelineThumbnailMode") == Timeline::kThumbnailOn) {
                                        ...
                                        if (height > 0) {  // Prevent divide by zero/invalid params
                                            ...
                                            start =
                                                (((preview_rect.left() - int(qFloor(block_in))) / thumb_rect.width()) *
                                                 thumb_rect.width()) +
                                                qFloor(block_in);
                                        } else {
                                        ...
                                        for (int i = start; i < preview_rect.right(); i += thumb_rect.width() + 1) {
                                            rational time_here = SceneToTime(i - block_in, GetScale(),
                                        ...
                                        time = Timecode::snap_time_to_timebase(time, thumbs->GetTimebase(),
                                                                               Timecode::kFloor);
                        ...
                        if (clip->GetTrackType() == Track::kAudio &&
                            ARCVIDEO_CONFIG("TimelineWaveformMode").toInt() == Timeline::kWaveformsEnabled) {
                        ...
                        if (clip->connected_viewer()) {
                            if (!clip->connected_viewer()->GetLength().isNull()) {
                                ...
                                if (clip->media_in() < 0) {
                                    ...
                                    switch (clip->loop_mode()) {
                                        case LoopMode::kLoopModeOff:
                                            ...
                                            if (zebra_right > GetTimelineLeftBound()) {
                                                DrawZebraStripes(painter,
                                                                 QRectF(block_left, block_top, zebra_right - block_left,
                                                                        block_height));
                                            }
                                            break;
                                        ...
                                        case LoopMode::kLoopModeClamp:
                                            painter->drawLine(zebra_right, block_top, zebra_right,
                                ...
                                if (clip->length() + clip->media_in() > clip->connected_viewer()->GetLength()) {
                                    qreal zebra_left =
                                        TimeToScene(clip->out() - (clip->media_in() + clip->length() -
                                    ...
                                    switch (clip->loop_mode()) {
                                        case LoopMode::kLoopModeOff:
                                            ...
                                            if (zebra_left < GetTimelineRightBound()) {
                                                DrawZebraStripes(painter,
                                                                 QRectF(zebra_left, block_top, block_right - zebra_left,
                                                                        block_height));
                                            }
                                            break;
                                        case LoopMode::kLoopModeLoop:
                                            ...
                                            break;
                                        case LoopMode::kLoopModeClamp:
                                            painter->drawLine(zebra_left, block_top, zebra_left,
                                                              block_top + block_height);
                                            break;
                                    }
                                }
                            ...
                            if (!marker_list->empty()) {
                                ...
                                for (auto it = marker_list->cbegin(); it != marker_list->cend(); it++) {
                                    ...
                                    if (marker->time().in() >= clip->media_in() &&
                                        ...
                                        QPoint marker_pt(
                                            TimeToScene(clip->in() - clip->media_in() + marker->time().in()),
                                            block_top + block_height);
                        ...
                        if (const FrameHashCache* cache = clip->connected_video_cache()) {
                            if (cache->HasValidatedRanges()) {
                                QRect cache_rect =
                                    r.adjusted(0, r.height() - PlaybackCache::GetCacheIndicatorHeight(), 0, 0).toRect();
