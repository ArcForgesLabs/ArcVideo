...
void TransitionTool::MouseRelease(TimelineViewMouseEvent* event) {
    ...
    if (ghost_) {
        if (!ghost_->GetAdjustedLength().isNull()) {
            ...
            command->add_child(new TrackPlaceBlockCommand(sequence()->track_list(track.type()), track.index(),
                                                          transition, ghost_->GetAdjustedIn()));
            ...
            if (dual_transition_) {
                ...
                auto* friend_block =
                    QtUtils::ValueToPtr<Block>(ghost_->GetData(TimelineViewGhostItem::kReferenceBlock));
                ...
                command->add_child(
                    new NodeEdgeAddCommand(out_block, NodeInput(transition, TransitionBlock::kOutBlockInput)));

                command->add_child(
                    new NodeEdgeAddCommand(in_block, NodeInput(transition, TransitionBlock::kInBlockInput)));
                ...
                auto* block_to_transition =
                    QtUtils::ValueToPtr<Block>(ghost_->GetData(TimelineViewGhostItem::kAttachedBlock));
