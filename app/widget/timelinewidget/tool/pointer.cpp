    ...
    if (trim_mode == Timeline::kMove) {
        ...
        if (slide_instead_of_moving) {
            ...
            for (auto i = earliest_block_on_track.constBegin(); i != earliest_block_on_track.constEnd(); i++) {
                ...
                {
                    TimelineViewGhostItem* earliest_ghost = nullptr;
                    ...
                    if (earliest->previous() && slide_with_earliest_previous) {
                        ...
                        earliest_ghost =
                            AddGhostFromNull(earliest->in(), earliest->in(), track->ToReference(), Timeline::kTrimOut);
                    }
                ...
                if (latest->next()) {
                    TimelineViewGhostItem* latest_ghost = nullptr;
                    ...
                    if (slide_with_latest_next) {
                        ...
                        latest_ghost =
                            AddGhostFromNull(latest->out(), latest->out(), track->ToReference(), Timeline::kTrimIn);
                    }
        ...
        for (Block* clip_item : clips) {
            ...
            if (cb) {
                ...
                TransitionBlock* connected_transition = nullptr;
            ...
            if (!dont_roll_trims) {
                ...
                if (adjacent) {
                    ...
                    adjacent_ghosts.append(AddGhostFromNull(null_ghost_pos, null_ghost_pos,
                                                            clip_item->track()->ToReference(), flipped_mode));
                }
...
void PointerTool::FinishDrag(TimelineViewMouseEvent* event) {
    ...
    if (!blocks_trimming.isEmpty()) {
        for (const GhostBlockPair& p : blocks_trimming) {
    ...
    if (!blocks_moving.isEmpty()) {
        ...
        if (!duplicate_clips) {
            ...
            arcvideo::TimelineWidget::ReplaceBlocksWithGaps(blocks_to_delete, false, command, false);
        }
        ...
        for (const GhostBlockPair& p : blocks_moving) {
            Block* block = p.block;

            if (duplicate_clips) {
                // Duplicate rather than move
                // Place the copy instead of the original block
                auto* new_block = static_cast<Block*>(Node::CopyNodeInGraph(block, command));
            ...
            command->add_child(new TrackPlaceBlockCommand(sequence()->track_list(track_ref.type()), track_ref.index(),
                                                          block, p.ghost->GetAdjustedIn()));
        }
        ...
        if (!relinks.empty()) {
            for (auto it = relinks.cbegin(); it != relinks.cend(); it++) {
                ...
                if (auto* og_clip = dynamic_cast<ClipBlock*>(it.key())) {
                    auto* cp_clip = static_cast<ClipBlock*>(it.value());
                    ...
                    if (og_in_transition && relinks.contains(og_in_transition)) {
                        auto* cp_in_transition = static_cast<TransitionBlock*>(relinks.value(og_in_transition));
                        command->add_child(new NodeEdgeAddCommand(
                            cp_clip, NodeInput(cp_in_transition, TransitionBlock::kInBlockInput)));
                    }

                    if (og_out_transition && relinks.contains(og_out_transition)) {
                        auto* cp_out_transition = static_cast<TransitionBlock*>(relinks.value(og_out_transition));
                        command->add_child(new NodeEdgeAddCommand(
                            cp_clip, NodeInput(cp_out_transition, TransitionBlock::kOutBlockInput)));
                    }
                }
            }
        }
    ...
    if (!blocks_sliding.isEmpty()) {
        ...
        for (const GhostBlockPair& p : blocks_sliding) {
