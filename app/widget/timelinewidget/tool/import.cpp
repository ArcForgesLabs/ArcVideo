...
ImportTool::ImportTool(TimelineWidget* parent) : TimelineTool(parent) {
...
void ImportTool::DragEnter(TimelineViewMouseEvent* event) {
    ...
    if (mime_formats.contains(Project::kItemMimeType)) {
        ...
        if (parent()->HasGhosts() || !parent()->GetConnectedNode()) {
            ...
            QObject::connect(event->GetMimeData(), &QObject::destroyed, parent(),
...
void ImportTool::DragMove(TimelineViewMouseEvent* event) {
    if (!dragged_footage_.isEmpty()) {
        if (parent()->HasGhosts()) {
            ...
            QString tooltip_text = QString::fromStdString(
                Timecode::time_to_timecode(earliest_ghost, tooltip_timebase, Core::instance()->GetTimecodeDisplay()));

            // Force tooltip to update (otherwise the tooltip won't move as written in the documentation, and could get
            // in the way of the cursor)
            QToolTip::hideText();
...
void ImportTool::PlaceAt(const QVector<ViewerOutput*>& footage, const rational& start, bool insert,
                         MultiUndoCommand* command, int track_offset, bool jump_to_end) {
...
void ImportTool::PlaceAt(const DraggedFootageData& footage, const rational& start, bool insert,
    ...
    rational end_point(0);
    if (jump_to_end) {
        for (TimelineViewGhostItem* ghost : parent()->GetGhostItems()) {
            end_point = (std::max)(end_point, ghost->GetAdjustedOut());
        }
    }
    ...
    if (jump_to_end) {
        this->sequence()->SetPlayhead(end_point);
    }
}

void ImportTool::FootageToGhosts(rational ghost_start, const DraggedFootageData& sorted, const rational& dest_tb,
    ...
    for (auto it = sorted.cbegin(); it != sorted.cend(); it++) {
        ...
        if (wk->enabled()) {
            ...
            if (footage_duration.isNull()) {
                // Fallback to still length if length was 0
                footage_duration = ARCVIDEO_CONFIG("DefaultStillLength").value<rational>();
            }
        }
...
void ImportTool::PrepGhosts(const rational& frame, const int& track_index) {
    if (parent()->GetConnectedNode()) {
        FootageToGhosts(frame, dragged_footage_, parent()->GetConnectedNode()->GetVideoParams().time_base(),
                        track_index);
    }
}
...
void ImportTool::DropGhosts(bool insert, MultiUndoCommand* parent_command) {
    ...
    if (sequence) {
        ...
        DropWithoutSequenceBehavior behavior =
            static_cast<DropWithoutSequenceBehavior>(ARCVIDEO_CONFIG("DropWithoutSequenceBehavior").toInt());
        ...
        if (behavior == kDWSAsk) {
            ...
            mbox.setText(QCoreApplication::translate("ImportTool",
            ...
            QPushButton* auto_params_btn = mbox.addButton(
                QCoreApplication::translate("ImportTool", "Automatically Detect Parameters From Footage"),
                QMessageBox::YesRole);
            QPushButton* manual_params_btn = mbox.addButton(
                QCoreApplication::translate("ImportTool", "Set Parameters Manually"), QMessageBox::NoRole);
            ...
            if (behavior != kDWSDisable && dont_ask_again_box->isChecked()) {
                ARCVIDEO_CONFIG("DropWithoutSequenceBehavior") = behavior;
            }
        }
        ...
        if (behavior != kDWSDisable) {
            ...
            if (active_project) {
                Sequence* new_sequence = arcvideo::Core::CreateNewSequenceForProject(active_project);
                ...
                if (sequence_is_valid) {
                    ...
                    command->add_child(
                        new FolderAddChild(Core::instance()->GetSelectedFolderInActiveProject(), new_sequence));
    ...
    if (dst_graph) {
        ...
        for (int i = 0; i < parent()->GetGhostItems().size(); i++) {
            ...
            if (track_type == Track::kVideo || track_type == Track::kAudio) {
                auto footage_stream = ghost->GetData(TimelineViewGhostItem::kAttachedFootage)
                ...
                switch (Track::Reference::TypeFromString(footage_stream.output)) {
                    case Track::kVideo: {
                        ...
                        command->add_child(
                            new NodeSetValueHintCommand(transform, TransformDistortNode::kTextureInput, -1,
                                                        Node::ValueHint({NodeValue::kTexture}, footage_stream.output)));

                        command->add_child(new NodeEdgeAddCommand(
                            footage_stream.footage, NodeInput(transform, TransformDistortNode::kTextureInput)));
                    ...
                    case Track::kAudio: {
                        ...
                        command->add_child(
                            new NodeSetValueHintCommand(volume_node, VolumeNode::kSamplesInput, -1,
                                                        Node::ValueHint({NodeValue::kSamples}, footage_stream.output)));
