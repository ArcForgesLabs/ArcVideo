            ...
            if (block_after_ripple) {
                ...
                if (dynamic_cast<GapBlock*>(block_after_ripple)) {
                    ...
                    if (dynamic_cast<GapBlock*>(previous)) {
                        ...
                        ghost = AddGhostFromNull(block_after_ripple->in(), block_after_ripple->in(),
                                                 track->ToReference(), trim_mode);
...
void RippleTool::FinishDrag(TimelineViewMouseEvent* event) {
    ...
    if (parent()->HasGhosts()) {
        ...
        if (command->child_count() > 0) {
            ...
            command->add_child(
                new TimelineWidget::SetSelectionsCommand(parent(), new_sel, parent()->GetSelections(), false));
