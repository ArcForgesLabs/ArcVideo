...
void ViewerWidget::UpdateAudioProcessor()
{
  if (GetConnectedNode()) {
    ...
    AudioParams packed(ARCVIDEO_CONFIG("AudioOutputSampleRate").toInt(),
                       ARCVIDEO_CONFIG("AudioOutputChannelLayout").toULongLong(),
...
void ViewerWidget::CreateAddableAt(const QRectF &f)
{
  if (Sequence *s = dynamic_cast<Sequence*>(GetConnectedNode())) {
    ...
    rational length = ARCVIDEO_CONFIG("DefaultStillLength").value<rational>();
...
{
  QFont f(ARCVIDEO_CONFIG("DefaultSubtitleFamily").toString(), ARCVIDEO_CONFIG("DefaultSubtitleSize").toInt(), ARCVIDEO_CONFIG("DefaultSubtitleWeight").toInt());
...
void ViewerWidget::ShowContextMenu(const QPoint &pos)
{
...

  {
    ...
    connect(stop_playback_on_last_frame, &QAction::triggered, this, [](bool e){
      ARCVIDEO_CONFIG("StopPlaybackOnLastFrame") = e;
    });
...
void ViewerWidget::Play(bool in_to_out_only)
{
  if (in_to_out_only) {
    ...
    recording_filename_ = audio_path.filePath(QStringLiteral("%1.%2").arg(
                                                ...
                                                ExportFormat::GetExtension(static_cast<ExportFormat::Format>(ARCVIDEO_CONFIG("AudioRecordingFormat").toInt())))
                                              );

    AudioParams ap(ARCVIDEO_CONFIG("AudioRecordingSampleRate").toInt(),
