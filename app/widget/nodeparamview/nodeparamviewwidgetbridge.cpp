...
void NodeParamViewWidgetBridge::WidgetCallback() {
    switch (GetDataType()) {
        ...
        case NodeValue::kInt: {
            ...
            auto* slider = static_cast<IntegerSlider*>(sender());
        ...
        case NodeValue::kFloat: {
            ...
            auto* slider = static_cast<FloatSlider*>(sender());
        ...
        case NodeValue::kRational: {
            ...
            auto* slider = static_cast<RationalSlider*>(sender());
            ProcessSlider(slider, QVariant::fromValue(slider->GetValue()));
            break;
        }
        case NodeValue::kVec2: {
        ...
        case NodeValue::kCombo: {
            ...
            auto* cb = static_cast<QComboBox*>(widgets_.first());
        ...
        case NodeValue::kBezier: {
            ...
            auto* bw = static_cast<BezierWidget*>(widgets_.first());
            auto* fs = static_cast<FloatSlider*>(sender());
...
void NodeParamViewWidgetBridge::UpdateWidgetValues() {
    ...
    switch (GetDataType()) {
        ...
        case NodeValue::kFile: {
            auto* ff = static_cast<FileField*>(widgets_.first());
        ...
        case NodeValue::kText: {
            auto* e = static_cast<NodeParamViewTextEdit*>(widgets_.first());
        ...
        case NodeValue::kBoolean:
            ...
            break;
        case NodeValue::kFont: {
            auto* fc = static_cast<QFontComboBox*>(widgets_.first());
...
void NodeParamViewWidgetBridge::SetProperty(const QString& key, const QVariant& value) {
    ...
    if (NodeValue::type_is_numeric(data_type) || NodeValue::type_is_vector(data_type)) {
        if (key == QStringLiteral("min")) {
            ...
            for (auto widget : widgets_) {
                static_cast<NumericSliderBase*>(widget)->SetDragMultiplier(d);
            }
        }
    }
    ...
    if (data_type == NodeValue::kCombo) {
        if (key == QStringLiteral("combo_str")) {
            ...
            // Block the combobox changed signals since we anticipate the index will be the same and not require a
            // re-render
            cb->blockSignals(true);
            ...
            // In case the amount of items is LESS and the previous index cannot be set, NOW we trigger a re-cache since
    ...
    if (data_type == NodeValue::kText) {
        auto* tex = static_cast<NodeParamViewTextEdit*>(widgets_.first());
...
void NodeParamViewWidgetBridge::UpdateProperties() {
    ...
    for (const auto& it : std::views::reverse(input_hierarchy_)) {
