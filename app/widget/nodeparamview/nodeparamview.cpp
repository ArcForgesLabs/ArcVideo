...
NodeParamView::NodeParamView(bool create_keyframe_view, QWidget* parent)
    ...
    for (int i = 0; i < context_items_.size(); i++) {
        ...
        auto* title_bar = static_cast<NodeParamViewItemTitleBar*>(c->titleBarWidget());
    ...
    connect(param_scroll_area_->verticalScrollBar(), &QScrollBar::valueChanged, vertical_scrollbar_,
            &QScrollBar::setValue);
    connect(param_scroll_area_->verticalScrollBar(), &QScrollBar::rangeChanged, vertical_scrollbar_,
            &QScrollBar::setRange);
    connect(param_scroll_area_->verticalScrollBar(), &QScrollBar::rangeChanged, this,
            &NodeParamView::UpdateGlobalScrollBar);
    connect(vertical_scrollbar_, &QScrollBar::valueChanged, param_scroll_area_->verticalScrollBar(),
            &QScrollBar::setValue);
    ...
    if (create_keyframe_view) {
        ...
        connect(keyframe_view_, &KeyframeView::Dragged, this,
                static_cast<void (NodeParamView::*)(int)>(&NodeParamView::SetCatchUpScrollValue));
        connect(keyframe_view_, &KeyframeView::Released, this,
                static_cast<void (NodeParamView::*)()>(&NodeParamView::StopCatchUpScrollTimer));
        ...
        connect(keyframe_view_->verticalScrollBar(), &QScrollBar::valueChanged, vertical_scrollbar_,
                &QScrollBar::setValue);
        connect(keyframe_view_->verticalScrollBar(), &QScrollBar::valueChanged, param_scroll_area_->verticalScrollBar(),
                &QScrollBar::setValue);
        connect(param_scroll_area_->verticalScrollBar(), &QScrollBar::valueChanged, keyframe_view_->verticalScrollBar(),
                &QScrollBar::setValue);
        connect(vertical_scrollbar_, &QScrollBar::valueChanged, keyframe_view_->verticalScrollBar(),
                &QScrollBar::setValue);
...
void NodeParamView::UpdateContexts() {
    ...
    if (changes_made) {
        ...
        if (IsGroupMode()) {
            ...
            auto* group = static_cast<NodeGroup*>(contexts_.first());
...
void NodeParamView::SelectNodeFromConnectedLink(Node* node) {
    auto* item = static_cast<NodeParamViewItem*>(sender());
...
void NodeParamView::SetSelectedNodes(const QVector<NodeParamViewItem*>& nodes, bool handle_focused_node,
                                     bool emit_signal) {
...
bool NodeParamView::Paste() {
    if (keyframe_view_) {
        if (keyframe_view_->Paste([this](auto&& PH1) { return GetNodeWithID(std::forward<decltype(PH1)>(PH1)); })) {
            return true;
        }
    }

    return Paste(this, [this](auto&& PH1) { return GenerateExistingPasteMap(std::forward<decltype(PH1)>(PH1)); });
}

bool NodeParamView::Paste(
    QWidget* parent, std::function<QHash<Node*, Node*>(const ProjectSerializer::Result&)> get_existing_map_function) {
...
void NodeParamView::InputArraySizeChanged(const QString& input, int, int new_size) {
    auto* sender = static_cast<NodeParamViewItem*>(this->sender());
