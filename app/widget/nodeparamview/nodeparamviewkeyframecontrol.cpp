...
rational NodeParamViewKeyframeControl::GetCurrentTimeAsNodeTime() const {
    return GetAdjustedTime(GetTimeTarget(), input_.node(), GetTimeTarget()->GetPlayhead(),
...
void NodeParamViewKeyframeControl::KeyframeEnableBtnClicked(bool e) {
    ...
    if (e) {
        ...
        if (QMessageBox::warning(this, tr("Warning"),
                                 tr("Are you sure you want to disable keyframing on this value? This will clear all "
                                    "existing keyframes."),
                                 QMessageBox::Yes | QMessageBox::No) == QMessageBox::Yes) {
            // Store value at this time, we'll set this as the persistent value later
            const QVector<QVariant>& stored_vals =
                input_.node()->GetSplitValueAtTime(input_, GetCurrentTimeAsNodeTime());
            ...
            command_name =
                tr("Disabled Keyframing On %1 - %2").arg(input_.node()->GetLabelAndName(), input_.GetInputName());
        } else {
