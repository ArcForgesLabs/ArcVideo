    ...
    for (const QString& input : node->inputs()) {
        ...
        if (!(n->GetInputFlags(input) & kInputFlagHidden)) {
            ...
            if (n->InputIsArray(input)) {
                ...
                array_layout->setContentsMargins(QtUtils::QFontMetricsWidth(fontMetrics(), QStringLiteral("    ")), 0,
                                                 0, 0);
...
void NodeParamViewItemBody::CreateWidgets(QGridLayout* layout, Node* node, const QString& input, int element, int row) {
    ...
    if (node->InputIsArray(input)) {
        if (element == -1) {
            ...
            connect(insert_element_btn, &NodeParamViewArrayButton::clicked, this,
                    &NodeParamViewItemBody::ArrayInsertClicked);
            connect(remove_element_btn, &NodeParamViewArrayButton::clicked, this,
                    &NodeParamViewItemBody::ArrayRemoveClicked);
        }
    }
    ...
    if (node->IsInputConnectable(input)) {
        ...
        layout->addWidget(ui_objects.connected_label, row, kWidgetStartColumn, 1,
                          kKeyControlColumn - kWidgetStartColumn);
    }
    ...
    for (auto i = input_ui_map_.begin(); i != input_ui_map_.end(); i++) {
        ...
        if (ic.IsArray() && ic.element() >= 0) {
            ...
            i.value().main_label->setText(
                tr("%1:").arg(ic.element() + ic.GetProperty(QStringLiteral("arraystart")).toInt()));
        } else {
...
void NodeParamViewItemBody::InputArraySizeChangedInternal(Node* node, const QString& input, int size) {
    ...
    if (size != array_ui.count) {
        auto* grid = static_cast<QGridLayout*>(array_ui.widget->layout());
        ...
        if (it.value().append_btn == sender()) {
            ...
            Core::instance()->undo_stack()->push(
                ...
                tr("Appended Array Element In %1 - %2")
                    .arg(real_input.node()->GetLabelAndName(), real_input.GetInputName()));
            break;
        }
    }
}
...
void NodeParamViewItemBody::ToggleArrayExpanded() {
    auto* bridge = static_cast<NodeParamViewWidgetBridge*>(sender());
