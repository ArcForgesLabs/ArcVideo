...
ColorValuesWidget::ColorValuesWidget(ColorManager* manager, QWidget* parent)
    ...
    // Create value tabs
    {
        ...
        // FIXME: Display -> Ref temporarily disabled due to OCIO crash (see ColorDialog::ColorSpaceChanged for more
        // info)
...
ColorValuesTab::ColorValuesTab(bool with_legacy_option, QWidget* parent) : QWidget(parent) {
    ...
    if (with_legacy_option) {
        ...
        legacy_box_->setChecked(ARCVIDEO_CONFIG("UseLegacyColorInInputTab").toBool());
...
void ColorValuesTab::LegacyChanged(bool legacy) {
    ARCVIDEO_CONFIG("UseLegacyColorInInputTab") = legacy;
