...
CurveView::CurveView(QWidget* parent) : KeyframeView(parent), dragging_bezier_pt_(nullptr) {
...
void CurveView::SelectKeyframesOfInput(const NodeKeyframeTrackReference& ref) {
    ...
    for (KeyframeViewInputConnection* con : track_connections_) {
        for (NodeKeyframe* key : con->GetKeyframes()) {
            SelectKeyframe(key);
        }
    }
}
...
void CurveView::drawBackground(QPainter* painter, const QRectF& rect) {
    ...
    // Draw keyframe lines
    for (const NodeKeyframeTrackReference& ref : connected_inputs_) {
        ...
        if (node->IsInputKeyframing(input, ref.input().element())) {
            ...
            if (!track.isEmpty()) {
                painter->setPen(QPen(keyframe_colors_.value(ref), qMax(1, fontMetrics().height() / 4)));
...
void CurveView::DrawKeyframe(QPainter* painter, NodeKeyframe* key, KeyframeViewInputConnection* track,
...
bool CurveView::FirstChanceMousePress(QMouseEvent* event) {
    ...
    for (const BezierPoint& b : bezier_pts_) {
    ...
    if (dragging_bezier_pt_) {
        ...
        dragging_bezier_point_start_ = (dragging_bezier_pt_->type == NodeKeyframe::kInHandle)
                                           ? key->bezier_control_in()
                                           : key->bezier_control_out();
        dragging_bezier_point_opposing_start_ = (dragging_bezier_pt_->type == NodeKeyframe::kInHandle)
                                                    ? key->bezier_control_out()
                                                    : key->bezier_control_in();
...
void CurveView::FirstChanceMouseMove(QMouseEvent* event) {
    ...
    QPointF new_bezier_pos =
        GenerateBezierControlPosition(dragging_bezier_pt_->type, dragging_bezier_point_start_, mouse_diff_scaled);
    ...
    if (!(event->modifiers() & Qt::ControlModifier)) {
        new_opposing_pos =
            GenerateBezierControlPosition(opposing_type, dragging_bezier_point_opposing_start_, -mouse_diff_scaled);
    } else {
        new_opposing_pos = dragging_bezier_point_opposing_start_;
    }

    dragging_bezier_pt_->keyframe->set_bezier_control(dragging_bezier_pt_->type, new_bezier_pos);

    dragging_bezier_pt_->keyframe->set_bezier_control(opposing_type, new_opposing_pos);
...
void CurveView::FirstChanceMouseRelease(QMouseEvent* event) {
    ...
    command->add_child(new KeyframeSetBezierControlPoint(
        ...
        dragging_bezier_pt_->keyframe->bezier_control(dragging_bezier_pt_->type), dragging_bezier_point_start_));

    if (!(event->modifiers() & Qt::ControlModifier)) {
        auto opposing_type = NodeKeyframe::get_opposing_bezier_type(dragging_bezier_pt_->type);

        command->add_child(new KeyframeSetBezierControlPoint(
            dragging_bezier_pt_->keyframe, opposing_type, dragging_bezier_pt_->keyframe->bezier_control(opposing_type),
...
void CurveView::KeyframeDragMove(QMouseEvent* event, QString& tip) {
    ...
    double scaled_diff = (mapToScene(event->position().toPoint()).y() - drag_start_.y()) / GetYScale();
    ...
    for (size_t i = 0; i < GetSelectedKeyframes().size(); i++) {
        ...
        key->set_value(FloatSlider::TransformDisplayToValue(
            FloatSlider::TransformValueToDisplay(drag_keyframe_values_.at(i).toDouble(), display) - scaled_diff,
            display));
    }
    ...
    if (ok) {
        ...
        tip.append(FloatSlider::ValueToString(num_value + GetOffsetFromKeyframe(tip_item),
...
QPointF CurveView::GenerateBezierControlPosition(const NodeKeyframe::BezierType mode, const QPointF& start_point,
        ...
        for (NodeKeyframe* key : con->GetKeyframes()) {
            if (!selected_only || IsKeyframeSelected(key)) {
                rational transformed_time =
                    GetAdjustedTime(key->parent(), GetTimeTarget(), key->time(), Node::kTransformTowardsOutput);
...
double CurveView::GetOffsetFromKeyframe(NodeKeyframe* key) {
    ...
    if (node->HasInputProperty(input, QStringLiteral("offset"))) {
        ...
        QVector<QVariant> track_vals =
            NodeValue::split_normal_value_into_track_values(node->GetInputDataType(input), v);
