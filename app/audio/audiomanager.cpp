...
bool AudioManager::PushToOutput(const AudioParams& params, const QByteArray& samples, QString* error) {
    ...
    if (output_params_ != params || output_stream_ == nullptr) {
        ...
        PaError r = Pa_OpenStream(&output_stream_, nullptr, &p, output_params_.sample_rate(),
                                  paFramesPerBufferUnspecified, paNoFlag, OutputCallback, output_buffer_);
        if (r != paNoError) {
            // Unhandled error
...
bool AudioManager::StartRecording(const EncodingParams& params, QString* error_str) {
    ...
    PaError r = Pa_OpenStream(&input_stream_, &p, nullptr, params.audio_params().sample_rate(),
                              paFramesPerBufferUnspecified, paNoFlag, InputCallback, input_encoder_);
    if (r == paNoError) {
...
PaDeviceIndex AudioManager::FindConfigDeviceByName(bool is_output_device) {
    ...
    return FindDeviceByName(ARCVIDEO_CONFIG_STR(entry).toString(), is_output_device);
}
...
AudioManager::AudioManager() : output_stream_(nullptr), input_stream_(nullptr), input_encoder_(nullptr) {
#ifdef PA_HAS_JACK
    // PortAudio doesn't do a strcpy, so we need a const char that's readily accessible (i.e. not
    // a QString converted to UTF-8)
    PaJack_SetClientName("ArcVideo");
#endif
