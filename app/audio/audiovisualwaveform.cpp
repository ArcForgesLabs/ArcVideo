...
void AudioVisualWaveform::OverwriteSamplesFromBuffer(const SampleBuffer& samples, int sample_rate,
    ...
    for (size_t i = 0; i < samples_length; i += channels_) {
        ...
        size_t src_end =
            qMin(size_t(qRound64((double(i + channels_) * chunk_size))) / channels_, samples.sample_count());
...
void AudioVisualWaveform::OverwriteSamples(const SampleBuffer& samples, int sample_rate, const rational& start) {
    ...
    while (true) {
        ...
        OverwriteSamplesFromMipmap(previous_mipmap->second, previous_mipmap->first.toDouble(), input_start,
                                   input_length, start - virtual_start_, current_mipmap->first.toDouble(),
...
void AudioVisualWaveform::DrawWaveform(QPainter* painter, const QRect& rect, const double& scale,
    ...
    bool rectified = ARCVIDEO_CONFIG("RectifiedWaveforms").toBool();
    ...
    for (size_t i = start; i < end; i++) {
        ...
        next_sample_index =
            std::min(arr.size(),
                     size_t(start_sample_index + std::floor(rate_dbl * static_cast<double>(i - rect.x() + 1) / scale) *
