...
CrashHandlerDialog::CrashHandlerDialog(const QString& report_path)
{
  ...
  layout->addWidget(new QLabel(tr("We're sorry, ArcVideo has crashed. Please help us fix it by "
  ...
  crash_widget_layout->setContentsMargins(0, 0, 0, 0);
...
QString CrashHandlerDialog::GetSymbolPath()
{
  ...
  app_path.cdUp();
  symbols_path = app_path.filePath(QStringLiteral("share/arcvideo-editor/symbols"));
...
void CrashHandlerDialog::SendErrorReport()
{
  ...
  request.setUrl(QStringLiteral("https://arcvideoeditor.org/crashpad/report.php"));
...
#if BUILDFLAG(IS_WIN)
  symbol_bin_name = QStringLiteral("arcvideo-editor.pdb");
#elif BUILDFLAG(IS_APPLE)
  symbol_bin_name = QStringLiteral("ArcVideo");
#else
  symbol_bin_name = QStringLiteral("arcvideo-editor");
#endif
  ...
  symbol_filename = QStringLiteral("ArcVideo.sym");
#else
  symbol_filename = QStringLiteral("arcvideo-editor.sym");
#endif
...
int main(int argc, char *argv[])
{
  ...
  arcvideo::CrashHandlerDialog chd(report);
  chd.open();
