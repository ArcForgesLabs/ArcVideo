...
#ifdef Q_OS_WINDOWS
// Qt5 workaround for menus in fullscreen (QWindowsWindowFunctions::setHasBorderInFullScreen)
// was removed in Qt6. The underlying bug is fixed in Qt6.
#endif
...
void Core::DeclareTypesForQt() {
    ...
    qRegisterMetaType<arcvideo::MainWindowLayoutInfo>();
    qRegisterMetaType<arcvideo::RenderTicketPtr>();
}
...
bool Core::AddOpenProjectFromTask(Task* task, bool add_to_recents) {
    auto* load_task = static_cast<ProjectLoadBaseTask*>(task);
...
void Core::ImportTaskComplete(Task* task) {
    auto* import_task = static_cast<ProjectImportTask*>(task);
...
#ifdef Q_OS_WINDOWS
    // Qt5 workaround removed: QWindowsWindowFunctions::setHasBorderInFullScreen is not available in Qt6
#endif

    // Start autorecovery timer using the config value as its interval
    SetAutorecoveryInterval(ARCVIDEO_CONFIG("AutorecoveryInterval").toInt());
...
void Core::SaveAutorecovery() {
    if (ARCVIDEO_CONFIG("AutorecoveryEnabled").toBool()) {
        if (open_project_ && !open_project_->has_autorecovery_been_saved()) {
            ...
            if (FileFunctions::DirectoryIsValid(project_autorecovery_dir)) {
                ...
                int64_t max_recoveries_per_file = ARCVIDEO_CONFIG("AutorecoveryMaximum").toLongLong();

                // Since we write an extra file, increment total allowed files by 1
                max_recoveries_per_file++;

                // Delete old entries
                QStringList recovery_files =
                    project_autorecovery_dir.entryList(QDir::Files | QDir::NoDotAndDotDot, QDir::Name);
                ...
                QMessageBox::critical(main_window_, tr("Auto-Recovery Error"),
                                      tr("Failed to save auto-recovery to \"%1\". "
                                         "ArcVideo may not have permission to this directory.")
...
Timecode::Display Core::GetTimecodeDisplay() {
    return static_cast<Timecode::Display>(ARCVIDEO_CONFIG("TimecodeDisplay").toInt());
}

void Core::SetTimecodeDisplay(Timecode::Display d) {
    ARCVIDEO_CONFIG("TimecodeDisplay") = d;
...
QString Core::GetProjectFilter(bool include_any_filter) {
    static const QVector<QPair<QString, QString>> FILTERS = {
        // Standard compressed ArcVideo project
        {tr("ArcVideo Project"), QStringLiteral("ove")},

        // Uncompressed XML ArcVideo project
        {tr("ArcVideo Project (Uncompressed XML)"), QStringLiteral("ovexml")},
...
void Core::SetStartupLocale() {
    ...
    QString use_locale = ARCVIDEO_CONFIG("Language").toString();
...
void Core::CheckForAutoRecoveries() {
    ...
    if (autorecovery_index.exists()) {
        ...
        if (autorecovery_index.open(QFile::ReadOnly)) {
            ...
            AutoRecoveryDialog ard(tr("The following projects had unsaved changes when ArcVideo "
...
void Core::WarnCacheFull() {
    if (!shown_cache_full_warning_ && main_window_) {
        ...
        QMessageBox::warning(main_window_, tr("Disk Cache Full"),
                             tr("The disk cache is currently full and ArcVideo is having to delete old "
...
bool Core::LabelNodes(const QVector<Node*>& nodes, MultiUndoCommand* parent) {
    ...
    QString s = QInputDialog::getText(main_window_, tr("Label Node"), tr("Set node label"), QLineEdit::Normal,
                                      start_label, &ok);
...
bool Core::CloseProject(bool auto_open_new, bool ignore_modified) {
    if (open_project_) {
        if (open_project_->is_modified() && !ignore_modified) {
            ...
            mb.setText(
                tr("The project '%1' has unsaved changes. Would you like to save them?").arg(open_project_->name()));
