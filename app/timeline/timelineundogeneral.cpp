...
void TrackReplaceBlockWithGapCommand::undo() {
    ...
    for (auto transition_remove_command : std::views::reverse(transition_remove_commands_)) {
...
void TimelineAddDefaultTransitionCommand::AddTransition(ClipBlock* c, CreateTransitionMode mode) {
    if (Track* t = c->track()) {
        ...
        if (t->type() == Track::kVideo) {
            p = NodeFactory::CreateFromID(ARCVIDEO_CONFIG("DefaultVideoTransition").toString());
        } else if (t->type() == Track::kAudio) {
            p = NodeFactory::CreateFromID(ARCVIDEO_CONFIG("DefaultAudioTransition").toString());
        }

        auto transition_length = ARCVIDEO_CONFIG("DefaultTransitionLength").value<rational>();
        ...
        if (transition_length > 0) {
            if (auto* transition = dynamic_cast<TransitionBlock*>(p)) {
                ...
                switch (mode) {
                    case kIn:
                        commands_.append(
                            new NodeEdgeAddCommand(c, NodeInput(transition, TransitionBlock::kInBlockInput)));
                        break;
                    case kOutDual:
                        commands_.append(
                            new NodeEdgeAddCommand(c->next(), NodeInput(transition, TransitionBlock::kInBlockInput)));
                        /* fall through */
                    case kOut:
                        commands_.append(
                            new NodeEdgeAddCommand(c, NodeInput(transition, TransitionBlock::kOutBlockInput)));
                        break;
                }
            }
        }
