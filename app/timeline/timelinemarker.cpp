...
TimelineMarker::TimelineMarker(QObject* parent) : color_(ARCVIDEO_CONFIG("MarkerColor").toInt()) {
    setParent(parent);
}
...
QRect TimelineMarker::Draw(QPainter* p, const QPoint& pt, int max_right, double scale, bool selected) {
    ...
    if (time_.out() != time_.in()) {
        ...
        QPoint points[] = {
            pt, QPoint(left, center_y), QPoint(left, top), QPoint(right, top), QPoint(right, center_y), pt,
        };
...
void TimelineMarkerList::HandleMarkerTimeChange() {
    auto* m = static_cast<TimelineMarker*>(sender());
...
MarkerAddCommand::MarkerAddCommand(TimelineMarkerList* marker_list, const TimeRange& range, const QString& name,
                                   int color)
...
MarkerAddCommand::MarkerAddCommand(TimelineMarkerList* marker_list, TimelineMarker* marker)
    : marker_list_(marker_list), added_marker_(marker) {
...
MarkerRemoveCommand::MarkerRemoveCommand(TimelineMarker* marker) : marker_(marker) {}
...
MarkerChangeColorCommand::MarkerChangeColorCommand(TimelineMarker* marker, int new_color)
    : marker_(marker), new_color_(new_color) {}
...
MarkerChangeTimeCommand::MarkerChangeTimeCommand(TimelineMarker* marker, const TimeRange& time,
                                                 const TimeRange& old_time)
    : marker_(marker), old_time_(old_time), new_time_(time) {}
