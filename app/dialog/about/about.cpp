  ...
  QDialog(parent)
{
  ...
  QLabel* icon = new QLabel(QStringLiteral("<html><img src=':/graphics/arcvideo-splash.png'></html>"));
  ...
  if (welcome_dialog || patrons.isEmpty()) {
    opening_statement = tr("<b>ArcVideo relies on support from the community to continue its development.</b>");
  } else {
    opening_statement = tr("ArcVideo wouldn't be possible without the support of gracious donations from the following people.");
  }

  QLabel* support_lbl = new QLabel(tr("<html>%1 "
                                        "If you like this project, please consider making a "
                                        "<a href='https://arcvideoeditor.org/donate.php'>one-time donation</a> or "
                                        "<a href='https://www.patreon.com/arcvideoeditor'>pledging monthly</a> to "
...
void AboutDialog::accept()
{
  if (dont_show_again_checkbox_ && dont_show_again_checkbox_->isChecked()) {
    ARCVIDEO_CONFIG("ShowWelcomeDialog") = false;
  }
