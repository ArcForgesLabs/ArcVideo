...
PreferencesAppearanceTab::PreferencesAppearanceTab()
{
...

  {
    ...
    for (int i=0; i<Node::kCategoryCount; i++) {
      ...
      ccc->SetColor(ARCVIDEO_CONFIG_STR(QStringLiteral("CatColor%1").arg(i)).toInt());
  ...
  row++;
  {
    ...
    marker_btn_->SetColor(ARCVIDEO_CONFIG("MarkerColor").toInt());
...
void PreferencesAppearanceTab::Accept(MultiUndoCommand *command)
{
  ...
  if (style_path != StyleManager::GetStyle()) {
    ...
    ARCVIDEO_CONFIG("Style") = style_path;
  }

  for (int i=0; i<color_btns_.size(); i++) {
    ARCVIDEO_CONFIG_STR(QStringLiteral("CatColor%1").arg(i)) = color_btns_.at(i)->GetSelectedColor();
  }

  ARCVIDEO_CONFIG("MarkerColor") = marker_btn_->GetSelectedColor();
}
