...
private slots:
     ...
     * @return
     *
     * Value used as function calls itself recursively to determine if a menu parent has any children that are not
     * hidden. If so, TRUE is returned so the parent is shown too (even if it doesn't match the search query). If not,
     * FALSE is returned so the parent is hidden.
     */
...
private:
    ...
    /**
     * @brief Internal function called by setup_kbd_shortcuts() to traverse down the menu bar's hierarchy and populate
     * the shortcut panel.
     *
