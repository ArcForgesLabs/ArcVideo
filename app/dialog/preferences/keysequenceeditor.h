...
public:
     ...
     *
     * Each of ArcVideo's menu actions has a unique string ID (that, unlike the text, is not translated) for matching
     * with an external shortcut configuration file. The ID is stored in the QAction's `property("id")`. This function
     * returns that ID.
     *
     * @return
     *
