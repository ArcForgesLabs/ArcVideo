...
void SequenceDialog::accept() {
    ...
    if (!VideoParams::FormatIsFloat(parameter_tab_->GetSelectedPreviewFormat()) &&
        !ARCVIDEO_CONFIG("PreviewNonFloatDontAskAgain").toBool()) {
        QMessageBox b(this);
        ...
        b.setText(
            tr("The preview resolution has been set to a non-float format. This may cause banding and clipping "
               "artifacts in the preview.\n\n"
        ...
        if (dont_show_again_->isChecked()) {
            ARCVIDEO_CONFIG("PreviewNonFloatDontAskAgain") = true;
        }
    }
...
void SequenceDialog::SetAsDefaultClicked() {
    if (QtUtils::MsgBox(this, QMessageBox::Question, tr("Confirm Set As Default"),
        ...
        ARCVIDEO_CONFIG("DefaultSequenceWidth") = parameter_tab_->GetSelectedVideoWidth();
        ARCVIDEO_CONFIG("DefaultSequenceHeight") = parameter_tab_->GetSelectedVideoHeight();
        ARCVIDEO_CONFIG("DefaultSequencePixelAspect") =
            QVariant::fromValue(parameter_tab_->GetSelectedVideoPixelAspect());
        ARCVIDEO_CONFIG("DefaultSequenceFrameRate") =
            QVariant::fromValue(parameter_tab_->GetSelectedVideoFrameRate().flipped());
        ARCVIDEO_CONFIG("DefaultSequenceInterlacing") = parameter_tab_->GetSelectedVideoInterlacingMode();
        ARCVIDEO_CONFIG("DefaultSequenceAudioFrequency") = parameter_tab_->GetSelectedAudioSampleRate();
        ARCVIDEO_CONFIG("DefaultSequenceAudioLayout") =
            QVariant::fromValue(parameter_tab_->GetSelectedAudioChannelLayout());
    }
}

SequenceDialog::SequenceParamCommand::SequenceParamCommand(Sequence* s, VideoParams video_params,
                                                           const AudioParams& audio_params, QString name,
                                                           bool autocache)
    : sequence_(s),
