...
TexturePtr OIIODecoder::RetrieveVideoInternal(const RetrieveVideoParams& p) {
    ...
    if (!buffer_.is_allocated() || last_params_.divider != p.divider) {
        ...
        if (p.divider == 1) {
            ...
            image_->read_image(0, 0, 0, -1, image_->spec().format, buf.localpixels(), buf.pixel_stride(),
...
bool OIIODecoder::FileTypeIsSupported(const QString& fn) {
    // We prioritize OIIO over FFmpeg to pick up still images more effectively, but some OIIO decoders (notably
    // OpenJPEG) will segfault entirely if given unexpected data (an MPEG-4 for instance). To workaround this issue, we
    // use OIIO's "extension_list" attribute and match it with the extension of the file.
...
VideoParams OIIODecoder::GetVideoParamsFromImageSpec(const OIIO::ImageSpec& spec) {
    ...
    video_params.set_format(
        OIIOUtils::GetFormatFromOIIOBasetype(static_cast<OIIO::TypeDesc::BASETYPE>(spec.format.basetype)));
