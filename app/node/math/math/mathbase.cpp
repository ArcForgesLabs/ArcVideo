...
ShaderCode MathNodeBase::GetShaderCodeInternal(const QString& shader_id, const QString& param_a_in,
    ...
    if (pairing == kPairTextureMatrix && op == kOpMultiply) {
        ...
        operation = operation.arg(GetShaderVariableCall(param_a_in, type_a), GetShaderVariableCall(param_b_in, type_b));
    }
...
QString MathNodeBase::GetShaderUniformType(const arcvideo::NodeValue::Type& type) {
    switch (type) {
...
QString MathNodeBase::GetShaderVariableCall(const QString& input_id, const NodeValue::Type& type,
...
void MathNodeBase::PushVector(NodeValueTable* output, arcvideo::NodeValue::Type type, const QVector4D& vec) const {
    switch (type) {
        case NodeValue::kVec2:
...
void MathNodeBase::ValueInternal(Operation operation, Pairing pairing, const QString& param_a_in,
    ...
    switch (pairing) {
        case kPairNumberNumber: {
            if (val_a.type() == NodeValue::kRational && val_b.type() == NodeValue::kRational && operation != kOpPower) {
                ...
                output->Push(NodeValue::kRational,
                             QVariant::fromValue(PerformAddSubMultDiv<rational, rational>(operation, val_a.toRational(),
                                                                                          val_b.toRational())),
                             this);
            } else {
                output->Push(NodeValue::kFloat,
                             PerformAll<float, float>(operation, RetrieveNumber(val_a), RetrieveNumber(val_b)), this);
            }
            break;
        }

        case kPairVecVec: {
            // We convert all vectors to QVector4D just for simplicity and exploit the fact that kVec4 is higher than
            // kVec2 in the enum to find the largest data type
            PushVector(
                output, qMax(val_a.type(), val_b.type()),
        ...
        case kPairColorColor: {
            ...
            output->Push(NodeValue::kColor, QVariant::fromValue(PerformAddSub<Color, Color>(operation, col_a, col_b)),
                         this);
            break;
        }
        ...
        case kPairSampleSample: {
            ...
            for (int i = 0; i < mixed_samples.audio_params().channel_count(); i++) {
                ...
                for (size_t j = 0; j < min_samples; j++) {
                    mixed_samples.data(i)[j] =
                        PerformAll<float, float>(operation, samples_a.data(i)[j], samples_b.data(i)[j]);
                }
            }
            ...
            if (max_samples > min_samples) {
                ...
                for (int i = 0; i < mixed_samples.audio_params().channel_count(); i++) {
                    memcpy(&mixed_samples.data(i)[min_samples], &larger_buffer.data(i)[min_samples],
        ...
        case kPairTextureMatrix: {
            ...
            job.SetShaderID(QStringLiteral("%1.%2.%3.%4")
                                .arg(QString::number(operation), QString::number(pairing),
                                     QString::number(val_a.type()), QString::number(val_b.type())));
            ...
            if (!texture) {
                ...
                QVector2D texture_res(texture->params().width() * texture->pixel_aspect_ratio().toDouble(),
                                      texture->params().height());

                QMatrix4x4 adjusted_matrix = TransformDistortNode::AdjustMatrixByResolutions(
                    number_val.toMatrix(), sequence_res, texture->params().offset(), texture_res);
...
void MathNodeBase::ProcessSamplesInternal(const NodeValueRow& values, MathNodeBase::Operation operation,
                                          ...
                                          const arcvideo::SampleBuffer& input, arcvideo::SampleBuffer& output,
                                          int index) {
...
MathNodeBase::PairingCalculator::PairingCalculator(const NodeValueTable& table_a, const NodeValueTable& table_b) {
    ...
    for (int i = 0; i < likelihoods.size(); i++) {
        if (likelihoods.at(i) > -1) {
            if (most_likely_pairing_ == kPairNone || likelihoods.at(i) > likelihoods.at(most_likely_pairing_)) {
