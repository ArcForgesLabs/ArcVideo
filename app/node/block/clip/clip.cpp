...
QString ClipBlock::id() const {
    return QStringLiteral("org.arcvideoeditor.ArcVideo.clip");
}
    ...
    if (type == Track::kVideo || type == Track::kAudio) {
        if (Node* connected = GetConnectedOutput(kBufferIn)) {
            ...
            if (type == Track::kVideo) {
                ...
                // Handle waveforms
                if (ARCVIDEO_CONFIG("TimelineWaveformMode").toInt() == Timeline::kWaveformsEnabled) {
...
bool ClipBlock::GetAdjustedThumbnailRange(TimeRange* r) const {
    switch (static_cast<Timeline::ThumbnailMode>(ARCVIDEO_CONFIG("TimelineThumbnailMode").toInt())) {
    ...
    if (from == kBufferIn) {
        ...
        if (new_connected_viewer != connected_viewer_) {
            if (connected_viewer_) {
                disconnect(connected_viewer_->GetMarkers(), &TimelineMarkerList::MarkerAdded, this,
            ...
            if (connected_viewer_) {
                connect(connected_viewer_->GetMarkers(), &TimelineMarkerList::MarkerAdded, this,
                        &ClipBlock::PreviewChanged);
                connect(connected_viewer_->GetMarkers(), &TimelineMarkerList::MarkerRemoved, this,
                        &ClipBlock::PreviewChanged);
                connect(connected_viewer_->GetMarkers(), &TimelineMarkerList::MarkerModified, this,
                        &ClipBlock::PreviewChanged);
            }
        }
...
void ClipBlock::LinkChangeEvent() {
    ...
    for (Node* n : links()) {
        auto* b = dynamic_cast<ClipBlock*>(n);
