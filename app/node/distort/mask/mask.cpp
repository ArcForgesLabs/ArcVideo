...
ShaderCode MaskDistortNode::GetShaderCode(const ShaderRequest& request) const {
    if (request.id == QStringLiteral("mrg")) {
        return {FileFunctions::ReadFileAsString(QStringLiteral(":/shaders/multiply.frag"))};
    } else if (request.id == QStringLiteral("feather")) {
        return {FileFunctions::ReadFileAsString(QStringLiteral(":/shaders/blur.frag"))};
    } else if (request.id == QStringLiteral("invert")) {
        return {FileFunctions::ReadFileAsString(QStringLiteral(":/shaders/invertrgba.frag"))};
    } else {
    ...
    if (texture) {
        ...
        if (value[kFeatherInput].toDouble() > 0.0) {
            ...
            feather.Insert(BlurFilterNode::kMethodInput,
                           NodeValue(NodeValue::kInt, int(BlurFilterNode::kGaussian), this));
            ...
            feather.Insert(BlurFilterNode::kRadiusInput,
                           NodeValue(NodeValue::kFloat, value[kFeatherInput].toDouble(), this));
            feather.SetIterations(2, BlurFilterNode::kTextureInput);
            feather.Insert(QStringLiteral("resolution_in"),
                           NodeValue(NodeValue::kVec2,
                                     texture ? texture->virtual_resolution() : globals.square_resolution(), this));

            merge.Insert(QStringLiteral("tex_b"),
                         NodeValue(NodeValue::kTexture, Texture::Job(job_params, feather), this));
        } else {
