...
QString Track::id() const {
    return QStringLiteral("org.arcvideoeditor.ArcVideo.track");
}
    ...
    for (const auto& it : arr) {
        ...
        if (samples_from_this_block.is_allocated()) {
            ...
            if (auto* clip_cast = dynamic_cast<ClipBlock*>(b)) {
                ...
                if (qIsNull(speed_value)) {
                    ...
                    if (clip_cast->maintain_audio_pitch()) {
                        ...
                        if (processor.Open(samples_from_this_block.audio_params(),
                                           samples_from_this_block.audio_params(), speed_value)) {
                            ...
                            if (r < 0) {
                                ...
                                if (!out.empty()) {
                                    int nb_samples =
                                        out.front().size() *
                                        samples_from_this_block.audio_params().bytes_per_sample_per_channel();
            ...
            for (int i = 0; i < samples_from_this_block.audio_params().channel_count(); i++) {
                block_range_buffer.set(i, samples_from_this_block.data(i) + source_offset, destination_offset,
                                       copy_length);
            }
        }
    }
...
void Track::RefreshBlockCacheFromArrayMap() {
    ...
    for (int i = 0; i < block_array_indexes_.size(); i++) {
        auto* b = static_cast<Block*>(GetConnectedOutput(kBlockInput, block_array_indexes_.at(i)));
...
void Track::BlockLengthChanged() {
    ...
    auto* b = static_cast<Block*>(sender());
