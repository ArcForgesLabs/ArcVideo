...
public:
    ...
    static TimeRange TransformRangeForBlock(const Block* block, const TimeRange& range) {
        return {TransformTimeForBlock(block, range.in()), TransformTimeForBlock(block, range.out())};
    }
    ...
    static TimeRange TransformRangeFromBlock(const Block* block, const TimeRange& range) {
        return {TransformTimeFromBlock(block, range.in()), TransformTimeFromBlock(block, range.out())};
    }
    ...
    [[nodiscard]] Reference ToReference() const { return {type(), Index()}; }
