...
ViewerOutput::ViewerOutput(bool create_buffer_inputs, bool create_default_streams)
    ...
    AddInput(kVideoParamsInput, NodeValue::kVideoParams,
             InputFlags(kInputFlagNotConnectable | kInputFlagNotKeyframable | kInputFlagArray | kInputFlagHidden));
...
QString ViewerOutput::id() const {
    return QStringLiteral("org.arcvideoeditor.ArcVideo.vieweroutput");
}
    ...
    switch (d) {
        case DURATION: {
            ...
            if (video.is_valid() && video.video_type() != VideoParams::kVideoTypeStill) {
                ...
                if (using_display == Timecode::kTimecodeDropFrame || using_display == Timecode::kTimecodeNonDropFrame) {
                    using_display = Timecode::kTimecodeSeconds;
                }

                using_timebase = audio.sample_rate_as_time_base();
            } else if (sub.is_valid()) {
                using_timebase = ARCVIDEO_CONFIG("DefaultSequenceFrameRate").value<rational>();
            }
...
void ViewerOutput::set_default_parameters() {
    int width = ARCVIDEO_CONFIG("DefaultSequenceWidth").toInt();
    int height = ARCVIDEO_CONFIG("DefaultSequenceHeight").toInt();

    SetVideoParams(VideoParams(width, height, ARCVIDEO_CONFIG("DefaultSequenceFrameRate").value<rational>(),
                               static_cast<PixelFormat::Format>(ARCVIDEO_CONFIG("OfflinePixelFormat").toInt()),
                               VideoParams::kInternalChannelCount,
                               ARCVIDEO_CONFIG("DefaultSequencePixelAspect").value<rational>(),
                               ARCVIDEO_CONFIG("DefaultSequenceInterlacing").value<VideoParams::Interlacing>(),
                               VideoParams::generate_auto_divider(width, height)));
    SetAudioParams(AudioParams(ARCVIDEO_CONFIG("DefaultSequenceAudioFrequency").toInt(),
                               ARCVIDEO_CONFIG("DefaultSequenceAudioLayout").toULongLong(), kDefaultSampleFormat));
}
...
void ViewerOutput::InputDisconnectedEvent(const QString& input, int element, Node* output) {
    if (input == kTextureInput) {
        ...
        disconnect(output->waveform_cache(), &AudioWaveformCache::Validated, this,
                   &ViewerOutput::ConnectedWaveformChanged);
    }
...
void ViewerOutput::InputValueChangedEvent(const QString& input, int element) {
    if (element == 0) {
        if (input == kVideoParamsInput) {
            ...
            bool size_changed = cached_video_params_.width() != new_video_params.width() ||
            ...
            bool pixel_aspect_changed =
                cached_video_params_.pixel_aspect_ratio() != new_video_params.pixel_aspect_ratio();
