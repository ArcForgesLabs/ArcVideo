...
NodeValue NodeTraverser::GenerateRowValue(const Node* node, const QString& input, NodeValueTable* table,
    ...
    if (value.array()) {
        ...
        for (auto& [key, tbl] : tables) {
            output[key] = GenerateRowValueElement(node, input, key, &tbl, time);
        }
...
int NodeTraverser::GenerateRowValueElementIndex(const Node* node, const QString& input, int element,
                                                const NodeValueTable* table) {
    return GenerateRowValueElementIndex(node->GetValueHintForInput(input, element), node->GetInputDataType(input),
                                        table);
}
                ...
                if (resolved_texture_cache_.contains(job_tex.get())) {
                    ...
                    if (auto* cj = dynamic_cast<CacheJob*>(base_job)) {
                        ...
                        rational footage_time = Footage::AdjustTimeByLoopMode(
                            fj->time().in(), fj->loop_mode(), fj->length(), fj->video_params().video_type(),
