...
Color Node::color() const {
    ...
    if (override_color_ >= 0) {
        ...
        c = ARCVIDEO_CONFIG_STR(QStringLiteral("CatColor%1").arg(this->Category().first())).toInt();
    }
...
QBrush Node::brush(qreal top, qreal bottom) const {
    if (ARCVIDEO_CONFIG("UseGradients").toBool()) {
        ...
        if (before) {
            if (before->time() == time ||
                ...
                if (before->type() == NodeKeyframe::kBezier && after->type() == NodeKeyframe::kBezier) {
                    ...
                    interpolated =
                        Bezier::QuadraticXtoY(time.toDouble(), Imath::V2d(before->time().toDouble(), before_val),
                                              control_point, Imath::V2d(after->time().toDouble(), after_val));

                } else {
                    // To have arrived here, the keyframes must both be linear
                    qreal period_progress = (time.toDouble() - before->time().toDouble()) /
                                            (after->time().toDouble() - before->time().toDouble());
...
void Node::InputArrayInsert(const QString& id, int index) {
    ...
    for (const auto& copied_edge : std::views::reverse(copied_edges)) {
            ...
            if (connection_index > -1) {
                ...
                if (command) {
                    ...
                    copied_input.node()->SetValueHintForInput(
                        copied_input.input(),
                        src_node->GetValueHintForInput(copied_input.input(), copied_input.element()),
...
Node* Node::CopyNodeAndDependencyGraphMinusItemsInternal(QMap<Node*, Node*>& created, Node* node,
    ...
    if (auto* src_group = dynamic_cast<NodeGroup*>(node)) {
        ...
        command->add_child(
            new NodeGroupSetOutputPassthrough(dst_group, created.value(src_group->GetOutputPassthrough())));
    }
...
void Node::ReportInvalidInput(const char* attempted_action, const QString& id, int element) const {
    qWarning() << "Failed to" << attempted_action << "parameter" << id << "element" << element << "in node"
               << this->id() << "- input doesn't exist";
}
...
bool Node::InputsFrom(Node* n, bool recursively) const {
    return std::ranges::any_of(input_connections_, [&](const auto& input_connection) {
        Node* connected = input_connection.second;
        return connected == n || (recursively && connected->InputsFrom(n, recursively));
    });
}
...
bool Node::InputsFrom(const QString& id, bool recursively) const {
    return std::ranges::any_of(input_connections_, [&](const auto& input_connection) {
        Node* connected = input_connection.second;
        return connected->id() == id || (recursively && connected->InputsFrom(id, recursively));
    });
}
...
TimeRange Node::TransformTimeTo(TimeRange time, Node* target, TransformTimeDirection dir, int path_index) {
    ...
    if (!path.empty()) {
        if (dir == kTransformTowardsInput) {
            for (const auto& i : std::views::reverse(path)) {
...
void Node::InvalidateFromKeyframeBezierInChange() {
    auto* key = static_cast<NodeKeyframe*>(sender());
        ...
        if (existing_key) {
            ...
            for (int i = 0; i < nb_tracks; i++) {
                ...
                if (i == track) {
                    ...
                    track_value =
                        input.node()->GetSplitValueAtTimeOnTrack(input.input(), node_time, i, input.element());
                }
