...
ProjectSerializer210528::LoadData ProjectSerializer210528::Load(Project* project, QXmlStreamReader* reader,
...

                    {
                        XMLAttributeLoop(reader, attr) {
                            if (attr.name() == QStringLiteral("id")) {
                                ...
                                is_cm = true;
                            } else if (attr.name() == QStringLiteral("settings") &&
    ...
    for (Node* n : project->nodes()) {
        ...
        if (auto* t = dynamic_cast<Track*>(n)) {
            for (int i = 0; i < t->InputArraySize(Track::kBlockInput); i++) {
                auto* b = static_cast<Block*>(t->GetConnectedOutput(Track::kBlockInput, i));
                if (!b->track()) {
                    t->AppendBlock(b);
                }
            }
            ...
            while (XMLReadNextStartElement(reader)) {
                if (reader->name() == QStringLiteral("connection")) {
                    ...
                    xml_node_data.desired_connections.append(
                        {NodeInput(node, param_id, ele), output_node_id.toULongLong(), output_param_id});
                } else {
                    reader->skipCurrentElement();
                }
            }
...
void ProjectSerializer210528::LoadColorManager(QXmlStreamReader* reader, Project* project) {
    while (XMLReadNextStartElement(reader)) {
        if (reader->name() == QStringLiteral("input")) {
            QString id;
            XMLAttributeLoop(reader, attr) {
...
void ProjectSerializer210528::LoadProjectSettings(QXmlStreamReader* reader, Project* project) {
    while (XMLReadNextStartElement(reader)) {
        if (reader->name() == QStringLiteral("input")) {
            QString id;
            XMLAttributeLoop(reader, attr) {
...
void ProjectSerializer210528::LoadImmediate(QXmlStreamReader* reader, Node* node, const QString& input, int element,
                ...
                if (reader->name() == QStringLiteral("track")) {
                    while (XMLReadNextStartElement(reader)) {
                        ...
                        if (reader->name() == QStringLiteral("key")) {
                            ...
                            auto* key =
                                new NodeKeyframe(key_time, key_value, key_type, track, element, key_input, node);
...
bool ProjectSerializer210528::LoadPosition(QXmlStreamReader* reader, quintptr* node_ptr, Node::Position* pos) {
...
void ProjectSerializer210528::PostConnect(const XMLNodeData& xml_node_data) {
...
void ProjectSerializer210528::LoadWorkArea(QXmlStreamReader* reader, TimelineWorkArea* workarea) {
...
void ProjectSerializer210528::LoadMarkerList(QXmlStreamReader* reader, TimelineMarkerList* markers) {
    while (XMLReadNextStartElement(reader)) {
        if (reader->name() == QStringLiteral("marker")) {
            ...
            new TimelineMarker(ARCVIDEO_CONFIG("MarkerColor").toInt(), TimeRange(in, out), name, markers);
        }
...
void ProjectSerializer210528::LoadValueHint(Node::ValueHint* hint, QXmlStreamReader* reader) {
