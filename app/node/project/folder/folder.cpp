...
bool Folder::HasChildRecursive(Node* child) const {
    return std::ranges::any_of(item_children_, [child](Node* i) {
        if (i == child) {
            return true;
        }
        if (auto* f = dynamic_cast<Folder*>(i)) {
            return f->HasChildRecursive(child);
        }
        return false;
    });
}
...
void Folder::RemoveElementCommand::redo() {
    if (!subcommand_) {
        ...
        if (remove_index_ != -1) {
            ...
            subcommand_->add_child(
                new NodeEdgeRemoveCommand(folder_->GetConnectedOutput(connected_input), connected_input));
