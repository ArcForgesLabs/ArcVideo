...
bool FileFunctions::CanCopyDirectoryWithoutOverwriting(const QString& source, const QString& dest) {
    ...
    return std::ranges::all_of(info_list, [&](const QFileInfo& info) {
        ...
        if (info.isDir()) {
            return CanCopyDirectoryWithoutOverwriting(info.absoluteFilePath(), dest_equivalent);
        }
        return !QFileInfo::exists(dest_equivalent);
    });
}
