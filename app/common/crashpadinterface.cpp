...
bool InitializeCrashpad() {
    QString report_path = QDir(arcvideo::FileFunctions::GetTempFilePath()).filePath(QStringLiteral("reports"));

    QString handler_fn = arcvideo::FileFunctions::GetFormattedExecutableForPlatform(QStringLiteral("crashpad_handler"));
    ...
    if (QFileInfo::exists(handler_abs_path)) {
        ...
        base::FilePath metrics_dir(QSTRING_TO_BASE_STRING(
            QDir(arcvideo::FileFunctions::GetTempFilePath()).filePath(QStringLiteral("metrics"))));
        ...
        status =
            client->StartHandler(handler, reports_dir, metrics_dir, "https://arcvideoeditor.org/crashpad/report.php",
