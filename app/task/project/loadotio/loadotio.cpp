...
bool LoadOTIOTask::Run() {
    ...
    for (auto timeline : timelines) {
        ...
        for (const auto& track : timeline->tracks()->children()) {
        ...
        for (const auto& c : timeline->tracks()->children()) {
            ...
            for (const auto& otio_block_retainer : clip_map) {
                ...
                if (otio_block->schema_name() == "Clip") {
                    ...
                    // Todo: Look into OTIO supported transitions and add them to ArcVideo
                ...
                if (otio_block->schema_name() == "Clip" || otio_block->schema_name() == "Gap") {
                    start_time = rational::fromDouble(
                        static_cast<OTIO::Item*>(otio_block)->source_range()->start_time().to_seconds());
                    duration = rational::fromDouble(
                        static_cast<OTIO::Item*>(otio_block)->source_range()->duration().to_seconds());
                ...
                if (prev_block_transition) {
                    auto* previous_transition_block = static_cast<TransitionBlock*>(previous_block);
                ...
                if (otio_block->schema_name() == "Transition") {
                    auto* transition_block = static_cast<TransitionBlock*>(block);
                    auto* otio_block_transition = static_cast<OTIO::Transition*>(otio_block);

                    // Set how far the transition eats into the previous clip
                    transition_block->set_offsets_and_length(
                        rational::fromRationalTime(otio_block_transition->in_offset()),
