        ...
        if (dynamic_cast<ClipBlock*>(block)) {
            ...
            if (!media_nodes.isEmpty()) {
                ...
                if (otio_track->kind() == "Video") {
                    ...
                    available_range =
                        OTIO::TimeRange(OTIO::RationalTime(0, media_nodes.first()->GetAudioParams().sample_rate()),
                                        OTIO::RationalTime(media_nodes.first()->GetAudioParams().duration(),
                                                           media_nodes.first()->GetAudioParams().sample_rate()));
                }
                auto media_ref =
                    new OTIO::ExternalReference(media_nodes.first()->filename().toStdString(), available_range);
            ...
            auto* our_transition = static_cast<TransitionBlock*>(block);
