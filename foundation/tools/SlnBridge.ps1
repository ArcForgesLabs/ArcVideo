<#
.SYNOPSIS
    Configure the CMake project and build the ArcVideoFoundation library.
.DESCRIPTION
    SlnBridge automates the workflow:
      1. cmake --preset <preset>   → generates the VS solution (.slnx)
      2. cmake --build             → builds the arcvideo-foundation library
         for both Debug and Release configs so the solution opens without errors.
...
.PARAMETER Configs
    Build configurations (default: Debug, Release).
    Can pass multiple, e.g. -Configs Debug,Release
.PARAMETER SkipConfigure
    Skip cmake configure and only build.
.PARAMETER Open
...
# Step 2: Build arcvideo-foundation
$foundationTargets = @("arcvideo-foundation")

Write-Log ">> Step 2: Building arcvideo-foundation..." Cyan
$targetList = $foundationTargets -join ', '
Write-Log "  Targets: $targetList" White

# 遍历所有的 Config (默认 Debug 和 Release 都跑一遍)
foreach ($cfg in $Configs) {
    Write-Log ">> Building Configuration: [$cfg]" Yellow
    foreach ($target in $foundationTargets) {
